...
/**
 * Update core contact info for a real user.
 *
 * `smsShiftAlertsOptIn` is optional: pass a boolean to set the column, or
 * omit (or pass undefined/null) to leave the existing value untouched. This
 * lets generic PATCH-style callers update only the fields they care about
 * without clobbering the shift-alert preference.
 *
 * @param {number} id
 * @param {{
 *   email: string,
 ...
 *   smsShiftAlertsOptIn?: boolean | null
 * }} contact
 * @param {string} [editedBy]
 * @returns {Promise<import("mssql").IResult<any>>}
 */
export async function updateUserContact(
  id,
  { email, phone, smsCapable, whatsappid, smsShiftAlertsOptIn },
  editedBy,
) {
  const tsql = `
    UPDATE dbo.volunteer_in
    SET email                   = @email,
        ...
        smsCapable              = @smsCapable,
        whatsappid              = @whatsappid,
        sms_shift_alerts_opt_in = COALESCE(@smsShiftAlertsOptIn, sms_shift_alerts_opt_in),
        last_updated            = SYSUTCDATETIME(),
        edited_by               = @editedBy
    WHERE id = @id;
  `;
  ...
  return exec(tsql, (req) => {
    ...
    // null → keep current column value (COALESCE in SQL); true/false → set.
    req.input(
      "smsShiftAlertsOptIn",
      sql.Bit,
      typeof smsShiftAlertsOptIn === "boolean"
        ? (smsShiftAlertsOptIn ? 1 : 0)
        : null,
    );
...
export async function getLocationsTasks(year) {
  const result = await exec(
    `
        SELECT
            lt.id, lt.year, lt.name, lt.type, lt.description, lt.capacity,
            lt.address, lt.lat, lt.lng, lt.maps_url, lt.active,
            lt.created_at, lt.created_by,
            lt.classification_id,
            svl.display_name AS classification_name
        FROM dbo.locations_tasks lt
        LEFT JOIN dbo.system_variable_lists svl
            ON svl.id = lt.classification_id
        WHERE lt.year = @year
        ORDER BY lt.type DESC, lt.name ASC;
    `,
    (req) => {
...
/**
 * Get all other schedule assignments at the same location for a given
 * year, so a rendezvous point can be copied to them. Excludes the
 * assignment the rendezvous point already belongs to. Flags whether
 * each candidate already has its own rendezvous point set.
 *
 * @param {number} locationTaskId
 * @param {number} excludeAssignmentId
 * @param {number} year
 * @returns {Promise<Array<{
 *   id:              number,
 *   shift_id:        number,
 ...
 * }>>}
 */
export async function getAssignmentsAtLocationForYear(locationTaskId, excludeAssignmentId, year) {
    const result = await exec(`
        SELECT
            sa.id, sa.shift_id, sh.label AS shift_label, sh.start_time,
            cd.label AS day_label, cd.convention_date,
            sc.name AS event_type_name,
            CASE WHEN rp.id IS NULL THEN 0 ELSE 1 END AS has_rendezvous
        FROM dbo.schedule_assignments sa
        JOIN dbo.shifts               sh   ON sh.id   = sa.shift_id
        JOIN dbo.sessions             sess ON sess.id = sh.session_id
        ...
        WHERE sa.location_task_id = @locationTaskId
          AND sa.id <> @excludeId
          AND cd.year = @year
        ORDER BY cd.convention_date, sh.start_time, sh.label;
    `, (req) => {
        req.input('locationTaskId', sql.Int, locationTaskId);
        req.input('excludeId',      sql.Int, excludeAssignmentId);
        req.input('year',           sql.Int, year);
    });
    return result.recordset || [];
}
...
export async function getShiftAttendanceData(shiftId) {
    const result = await exec(`
        -- RSVP info, deduplicated across campaigns (responded rows first,
        -- then most recent by id, so follow-up campaigns don't win over
        -- an actual response).
        ...
        -- Who's actually expected for this shift. Primary source is the
        -- Scheduler's shift_slot_assignments (the current assignment
        -- mechanism); campaign invitations are unioned in too since a
        -- volunteer can be invited without yet holding a Scheduler slot.
        expected AS (
            SELECT DISTINCT ssa.volunteer_id
            FROM dbo.shift_slot_assignments ssa
            JOIN dbo.schedule_assignments sa ON sa.id = ssa.schedule_assignment_id
            WHERE sa.shift_id = @shiftId
        ...
        FROM expected e
        INNER JOIN dbo.volunteer_in v ON v.id = e.volunteer_id
        LEFT JOIN ranked_invites ri
            ON ri.volunteer_id = e.volunteer_id
           AND ri.rn = 1
        LEFT JOIN dbo.attendance a
            ON a.volunteer_id = e.volunteer_id
           AND a.shift_id     = @shiftId

        UNION ALL

        -- Walk-ins: attendance rows with no matching Scheduler slot or invitation
        SELECT
            v.id            AS volunteer_id,
            v.firstName,
            v.lastName,
        ...
        WHERE a.shift_id = @shiftId
          ...
          AND NOT EXISTS (
              SELECT 1
              FROM dbo.shift_slot_assignments ssa2
              JOIN dbo.schedule_assignments sa2 ON sa2.id = ssa2.schedule_assignment_id
              WHERE sa2.shift_id     = @shiftId
                AND ssa2.volunteer_id = a.volunteer_id
          )
          AND NOT EXISTS (
              SELECT 1
 ...
 *
 * "Expected" volunteers are computed the same way as getShiftAttendanceData():
 * the Scheduler's shift_slot_assignments is the primary source, with campaign
 * invitations unioned in, since a volunteer can hold a Scheduler slot without
 * ever having been sent a formal invitation. invited_count/no_show_count are
 * driven off this combined set rather than dbo.invitations alone.
 *
 * @param {number} dayId
 * @returns {Promise<Array<{
 *   shift_id:         number,
    ...
    const result = await exec(
      `
        SELECT
            ...
            se.id                                                       AS session_id,
            se.label                                                    AS session_label,
            COUNT(DISTINCT e.volunteer_id)                              AS invited_count,
            ...
            COUNT(DISTINCT CASE
                WHEN e.volunteer_id IS NOT NULL
                 AND COALESCE(a.attended, 0) = 0
                 AND COALESCE(i.response, '') <> 'no'
                THEN e.volunteer_id END)                                AS no_show_count
        FROM dbo.shifts sh
        ...
        LEFT JOIN dbo.scheduler_categories sc
            ON sc.id = sh.category_id
        LEFT JOIN (
            -- Who's actually expected for this shift — Scheduler slot
            -- assignments primary, campaign invitations unioned in.
            -- Mirrors the "expected" CTE in getShiftAttendanceData().
            ...
            FROM dbo.invitations
            WHERE revoked = 0
        ) e ON e.shift_id = sh.id
        LEFT JOIN dbo.invitations i
            ON i.shift_id      = sh.id
           AND i.revoked       = 0
           AND i.volunteer_id  = e.volunteer_id
        LEFT JOIN dbo.attendance a
            ON a.shift_id      = sh.id
...
/**
 * Get a volunteer's saved scheduler column preferences (department column
 * order, hidden departments, and meeting-column hidden state).
 *
 * @param {number} volunteerId
 * @returns {Promise<{order:string[], hidden:string[], meetingHidden:boolean}|null>}
 */
export async function getSchedulerColumnPrefs(volunteerId) {
    const result = await exec(`
        SELECT scheduler_column_prefs
        FROM dbo.volunteer_in
        WHERE id = @id;
    `, (req) => {
        req.input('id', sql.Int, volunteerId);
    });
    const raw = result.recordset[0]?.scheduler_column_prefs;
    if (!raw) return null;
    try {
...
/**
 * Save a volunteer's scheduler column preferences.
 *
 * @param {number} volunteerId
 * @param {{order:string[], hidden:string[], meetingHidden:boolean}} prefs
 * @returns {Promise<boolean>} True if a row was updated.
 */
export async function updateSchedulerColumnPrefs(volunteerId, prefs) {
    const result = await exec(`
        UPDATE dbo.volunteer_in
        SET scheduler_column_prefs = @prefs
        WHERE id = @id;
    `, (req) => {
        req.input('id', sql.Int, volunteerId);
        req.input('prefs', sql.NVarChar(1000), JSON.stringify(prefs));
    });
    const affected = Array.isArray(result.rowsAffected)
        ? result.rowsAffected.reduce((s, n) => s + n, 0)
        : result.rowsAffected || 0;
...
export async function getPublishNotificationData(dayId) {
    ...
    for (const v of overseerResult.recordset || []) {
        if (!recipientMap.has(v.id)) {
            ...
            // Overseer who is also scheduled — stamp role onto the existing entry
            // so differential mode can always identify oversight-level recipients.
            recipientMap.get(v.id).role = v.role || null;
        }
    }
...
// ─────────────────────────────────────────────────────────────
// SCHEDULE PUBLISH SNAPSHOTS
// ─────────────────────────────────────────────────────────────

/**
 * Return the raw assignment rows for a convention day, suitable for
 * saving as a publish snapshot and diffing against a prior one.
 * Each row uniquely identifies one volunteer-slot assignment by its
 * foreign-key triple: (volunteer_id, shift_id, location_task_id).
 *
 * @param {number} dayId
 * @returns {Promise<Array<{
 *   volunteer_id:     number,
 ...
 * }>>}
 */
export async function getCurrentAssignmentSnapshot(dayId) {
    const result = await exec(
        `SELECT
             ssa.volunteer_id,
             sa.shift_id,
...
/**
 * Return the assignment snapshot from the most recent successful publish
 * for a given convention day, including readable labels and volunteer
 * contact details needed to notify removed volunteers.
 *
 * Returns an empty array when the day has never been published.
 *
 * @param {number} dayId
 * @returns {Promise<Array<{
 *   volunteer_id:     number,
 ...
 * }>>}
 */
export async function getLastPublishSnapshot(dayId) {
    const lastPublish = await exec(
        `SELECT TOP 1 id
         FROM dbo.schedule_publishes
         WHERE convention_day_id = @dayId
         ORDER BY id DESC;`,
        (req) => req.input('dayId', sql.Int, dayId),
    );

    const row = lastPublish.recordset?.[0];
    if (!row) return [];
    ...
    const result = await exec(
        `SELECT
             ...
             v.phone,
             v.smsCapable
         FROM dbo.schedule_publish_snapshots sps
         JOIN dbo.shifts sh          ON sh.id = sps.shift_id
         JOIN dbo.locations_tasks lt ON lt.id = sps.location_task_id
         JOIN dbo.volunteer_in v     ON v.id  = sps.volunteer_id
         WHERE sps.publish_id = @publishId;`,
        (req) => req.input('publishId', sql.Int, row.id),
    );
...
/**
 * Persist the assignment rows captured at publish time as a snapshot
 * linked to the given publish record.
 *
 * Rows are written in a single batched INSERT (chunked at 500 rows per
 * statement to stay under SQL Server's 2100-parameter limit — 5 params
 * per row × 500 = 2500 was too many; 400 rows × 5 = 2000 stays safe).
 *
 * @param {number} publishId - FK → dbo.schedule_publishes.id
 * @param {Array<{
 *   volunteer_id:     number,
 *   shift_id:         number,
 *   location_task_id: number,
 ...
 * @returns {Promise<void>}
 */
export async function savePublishSnapshot(publishId, rows) {
    if (!rows.length) return;

    const CHUNK_SIZE = 400; // 400 rows × 5 params = 2000, under the 2100 limit
    for (let start = 0; start < rows.length; start += CHUNK_SIZE) {
        const chunk = rows.slice(start, start + CHUNK_SIZE);
        const valuesSql = chunk
            .map(
                (_, i) =>
                    `(@publishId, @volunteerId${i}, @shiftId${i}, @locationTaskId${i}, @slotType${i})`,
            )
            .join(', ');

        await exec(
            `INSERT INTO dbo.schedule_publish_snapshots
                 (publish_id, volunteer_id, shift_id, location_task_id, slot_type)
             VALUES ${valuesSql};`,
            (req) => {
                req.input('publishId', sql.Int, publishId);
                chunk.forEach((r, i) => {
                    req.input(`volunteerId${i}`,    sql.Int,          r.volunteer_id);
                    req.input(`shiftId${i}`,       sql.Int,          r.shift_id);
                    req.input(`locationTaskId${i}`, sql.Int,          r.location_task_id);
                    req.input(`slotType${i}`,      sql.NVarChar(50), r.slot_type ?? null);
                });
            },
        );
    }
...
/**
 * Return the most recent publish record for each convention day that
 * has been published at least once.
 *
 * Used by GET /api/scheduler/publish-history to populate the publish
 * modal's last-published timestamps.
 *
 * @returns {Promise<Array<{
 *   convention_day_id: number,
...
export async function getPublishHistory() {
    const result = await exec(
        `SELECT
             sp.convention_day_id,
             sp.published_at,
             sp.filename
         FROM dbo.schedule_publishes sp
         INNER JOIN (
             SELECT convention_day_id, MAX(id) AS latest_id
             FROM   dbo.schedule_publishes
             GROUP BY convention_day_id
         ) latest ON latest.latest_id = sp.id
         ORDER BY sp.convention_day_id;`,
    );
    return result.recordset || [];
}
...
/**
 * Create a new magic login token record for a volunteer.
 * The raw token itself is never stored -- only its hash.
 *
 * @param {{
 *   volunteerId: number,
 *   tokenHash:   string,
 ...
 * @returns {Promise<number>} New row id.
 */
export async function createMagicLoginToken(data) {
    const result = await exec(`
        INSERT INTO dbo.magic_login_tokens
            (volunteer_id, token_hash, label, expires_at)
        OUTPUT INSERTED.id
        VALUES
            (@volunteerId, @tokenHash, @label, @expiresAt);
    `, (req) => {
        req.input('volunteerId', sql.Int,          data.volunteerId);
        req.input('tokenHash',   sql.Char(64),      data.tokenHash);
        req.input('label',       sql.NVarChar(100), data.label     || null);
        req.input('expiresAt',   sql.DateTime2,      data.expiresAt || null);
    });
    return result.recordset?.[0]?.id;
}

/**
 * Look up an active (non-revoked, non-expired) magic login token by
 * its hash, joined to the volunteer fields needed to establish a
 * session -- field names deliberately match what the password login
 * query returns so both can flow through the same session-setting
 * helper in accountRoutes.js.
 *
 * @param {string} tokenHash
 * @returns {Promise<object|null>}
 */
export async function getActiveMagicLoginTokenByHash(tokenHash) {
    const result = await exec(`
        SELECT
            mlt.id AS tokenId, mlt.expires_at, mlt.revoked_at,
            v.id, v.email, v.firstName, v.lastName,
            v.registration_status, v.account_status, v.role,
            v.extra_signs_placement, v.extra_parking_count
        FROM dbo.magic_login_tokens mlt
        JOIN dbo.volunteer_in v ON v.id = mlt.volunteer_id
        WHERE mlt.token_hash = @tokenHash
          AND mlt.revoked_at IS NULL
          AND (mlt.expires_at IS NULL OR mlt.expires_at > SYSUTCDATETIME());
    `, (req) => req.input('tokenHash', sql.Char(64), tokenHash));
    return result.recordset?.[0] || null;
}

/**
 * Stamp last_used_at on a magic login token after a successful login.
 *
 * @param {number} tokenId
 * @returns {Promise<void>}
 */
export async function touchMagicLoginToken(tokenId) {
    await exec(
        `UPDATE dbo.magic_login_tokens SET last_used_at = SYSUTCDATETIME() WHERE id = @id;`,
        (req) => req.input('id', sql.Int, tokenId),
    );
}
 ...
 */
export async function revokeMagicLoginToken(tokenId) {
    const result = await exec(
        `UPDATE dbo.magic_login_tokens SET revoked_at = SYSUTCDATETIME()
...
/**
 * List all magic login tokens for a volunteer (active and revoked),
 * most recent first. Useful for auditing which printed QR codes are
 * still live.
 *
 * @param {number} volunteerId
 * @returns {Promise<Array<object>>}
 */
export async function listMagicLoginTokensForVolunteer(volunteerId) {
    const result = await exec(`
        SELECT id, label, created_at, expires_at, revoked_at, last_used_at
        FROM dbo.magic_login_tokens
        WHERE volunteer_id = @volunteerId
...
/**
 * List every magic login token across all volunteers, most recent
 * first, joined to volunteer name/email for display. Used by the
 * Oversight Tools "Magic Links" admin page.
 *
 * @returns {Promise<Array<{
 *   id:           number,
 *   label:        string|null,
 ...
 * }>>}
 */
export async function listAllMagicLoginTokens() {
    const result = await exec(`
        SELECT
            mlt.id, mlt.label, mlt.created_at, mlt.expires_at,
            mlt.revoked_at, mlt.last_used_at,
            v.id AS volunteer_id, v.firstName, v.lastName, v.email
        FROM dbo.magic_login_tokens mlt
        JOIN dbo.volunteer_in v ON v.id = mlt.volunteer_id
...
/**
 * Return the most recent published schedule for each convention day,
 * grouped for the /schedules volunteer-facing page.
 *
 * The sharepoint_url column stores the blob download URL since the
 * 2.74.0 publish pipeline change, making links auth-free for volunteers.
 *
 * @returns {Promise<Array<{
 *   convention_day_id: number,
 *   day_label:         string,
...
export async function getPublishedSchedules() {
    const result = await exec(
        `SELECT
             ...
             sp.published_at,
             sp.filename,
             sp.sharepoint_url                             AS download_url
         FROM dbo.schedule_publishes sp
         INNER JOIN (
             SELECT convention_day_id, MAX(id) AS latest_id
             FROM   dbo.schedule_publishes
         ...
         JOIN dbo.convention_days cd ON cd.id = sp.convention_day_id
         WHERE sp.sharepoint_url IS NOT NULL
...
/**
 * Return all synced Maps resource files, ordered for grouping into tiles
 * by folder on the /maps page.
 *
 * @returns {Promise<Array<{
 *   id:             number,
 *   folder_name:    string,
...
export async function getMapFiles() {
    const result = await exec(`
        SELECT id, folder_name, file_name, blob_name, description,
               mime_type, size, source_item_id, scribble_url, embed_url,
               last_modified
        FROM dbo.map_files
...
/**
 * Return the set of Graph drive item IDs currently tracked in map_files.
 * Used by lib/mapsSync.js to detect files that have been removed from
 * the source SharePoint folder since the last sync.
 *
 * @returns {Promise<string[]>}
 */
export async function getMapFileSourceIds() {
    const result = await exec(`SELECT source_item_id FROM dbo.map_files;`);
    return (result.recordset || []).map((r) => r.source_item_id);
}

/**
 * Insert or update a synced Maps resource file record, keyed by
 * source_item_id (the Graph drive item ID).
 *
 * @param {{
 *   sourceItemId: string,
 *   folderName:   string,
 ...
 *   size:         number|null,
 *   scribbleUrl:  string|null,
 *   embedUrl:     string|null,
 *   lastModified: string|null,
 * }} data
 * @returns {Promise<void>}
 */
export async function upsertMapFile(data) {
    await exec(`
        MERGE dbo.map_files AS target
        USING (SELECT @sourceItemId AS source_item_id) AS src
            ON target.source_item_id = src.source_item_id
        WHEN MATCHED THEN
            UPDATE SET
                ...
                mime_type     = @mimeType,
                size          = @size,
                scribble_url  = @scribbleUrl,
                embed_url     = @embedUrl,
                last_modified = @lastModified,
                synced_at     = SYSUTCDATETIME()
        WHEN NOT MATCHED THEN
            INSERT (folder_name, file_name, blob_name, description, mime_type,
                    size, source_item_id, scribble_url, embed_url, last_modified)
            VALUES (@folderName, @fileName, @blobName, @description, @mimeType,
                    @size, @sourceItemId, @scribbleUrl, @embedUrl, @lastModified);
    `, (req) => {
        req.input('sourceItemId', sql.NVarChar(255),  data.sourceItemId);
        req.input('folderName',   sql.NVarChar(255),  data.folderName);
        req.input('fileName',     sql.NVarChar(255),  data.fileName);
        req.input('blobName',     sql.NVarChar(500),  data.blobName);
        req.input('description',  sql.NVarChar(500),  data.description  || null);
        req.input('mimeType',     sql.NVarChar(100),  data.mimeType     || null);
        req.input('size',         sql.BigInt,          data.size ?? null);
        req.input('scribbleUrl',  sql.NVarChar(1000), data.scribbleUrl  || null);
        req.input('embedUrl',     sql.NVarChar(1000), data.embedUrl     || null);
        req.input('lastModified', sql.DateTime2(0),    data.lastModified || null);
    });
}

/**
 * Delete map_files rows whose source_item_id is NOT in the given list.
 * Called by lib/mapsSync.js after each sync pass so files removed from
 * SharePoint are removed locally too. Callers are responsible for
 * deleting the corresponding blobs first (see deleteMapFileBlob).
 *
 * @param {string[]} keepSourceItemIds - IDs currently present in SharePoint.
 * @returns {Promise<Array<{ source_item_id: string, blob_name: string }>>}
 *          The rows that were deleted, so callers can clean up their blobs.
 */
export async function deleteMapFilesNotInSourceIds(keepSourceItemIds) {
    if (!keepSourceItemIds || keepSourceItemIds.length === 0) {
        const all = await exec(`SELECT source_item_id, blob_name FROM dbo.map_files;`);
        await exec(`DELETE FROM dbo.map_files;`);
        return all.recordset || [];
    }

    // Pass the ID list as a JSON array and unpack it with OPENJSON rather
    // than a table-valued parameter -- TVPs require a CREATE TYPE ... AS
    // TABLE declared on the server first, which this codebase doesn't use
    // anywhere else. OPENJSON needs no server-side type and works directly
    // against a bound NVARCHAR(MAX) parameter.
    const keepIdsJson = JSON.stringify(keepSourceItemIds);

    const toDelete = await exec(`
        SELECT source_item_id, blob_name
        FROM dbo.map_files
        WHERE source_item_id NOT IN (
            SELECT [value] FROM OPENJSON(@keepIds)
        );
    `, (req) => {
        req.input('keepIds', sql.NVarChar(sql.MAX), keepIdsJson);
    });

    await exec(`
        DELETE FROM dbo.map_files
        WHERE source_item_id NOT IN (
            SELECT [value] FROM OPENJSON(@keepIds)
        );
    `, (req) => {
 ...
 *
 * If fire_date is changing, this schedule's shift_alert_log is cleared
 * as a side effect so the corrected date fires cleanly -- otherwise
 * stale "sent" rows from the old (wrong) date silently suppress the
 * corrected run. See CHANGELOG.md for the 2026-07-03 incident (schedule
 * 8 firing 2026-06-30 instead of its intended date) that motivated this.
 *
 * @param {number} id
 * @param {{
 *   name:              string,
...
export async function updateAlertSchedule(id, data) {
    // Fetch the current fire_date before overwriting it, to detect
    // whether it's changing (see file-level note above).
    const current = await exec(
        `SELECT fire_date FROM dbo.shift_alert_schedules WHERE id = @id;`,
        (req) => { req.input('id', sql.Int, id); }
    );
    const oldFireDate = current.recordset?.[0]?.fire_date ?? null;
    const newFireDate = data.fire_date || null;
    const fireDateChanged =
        (oldFireDate == null) !== (newFireDate == null) ||
        (oldFireDate != null && newFireDate != null &&
            new Date(oldFireDate).toISOString().slice(0, 10) !==
            new Date(newFireDate).toISOString().slice(0, 10));
...
export async function getSchedulingCoverageSummary(year) {
    const result = await exec(
        `SELECT
             ...
             cd.label                                           AS day_label,
             CONVERT(VARCHAR(10), cd.convention_date, 120)      AS convention_date,
             COALESCE(needed.total_needed, 0)                   AS total_needed,
             COALESCE(assigned.total_assigned, 0)               AS total_assigned
         FROM dbo.convention_days cd
         LEFT JOIN (
             SELECT   sess.convention_day_id,
                      SUM(sa.volunteer_need) AS total_needed
             FROM     dbo.sessions             sess
             JOIN     dbo.shifts               sh  ON sh.session_id = sess.id
             JOIN     dbo.schedule_assignments sa  ON sa.shift_id   = sh.id
             GROUP BY sess.convention_day_id
         ) needed   ON needed.convention_day_id   = cd.id
         LEFT JOIN (
             SELECT   convention_day_id,
                      COUNT(id)              AS total_assigned
             FROM     dbo.shift_slot_assignments
             GROUP BY convention_day_id
         ) assigned ON assigned.convention_day_id = cd.id
         WHERE  cd.year = @year
         ORDER  BY cd.convention_date;`,
        (req) => {
            req.input('year', sql.Int, year);
...
/**
 * Fetch all active locations for a given year that have a capacity value set.
 * Used to populate the location picker on the Parking Counter page.
 *
 * @param {number} year
 * @returns {Promise<Array<{ id: number, name: string, capacity: number }>>}
 */
export async function getActiveLocationsForYear(year) {
    const result = await exec(
...
/**
 * Return the convention day whose date matches the supplied ISO date string.
 * Returns null when today is not a scheduled convention day.
 *
 * @param {number} year
 * @param {string} todayIso  Local date in YYYY-MM-DD format.
 * @returns {Promise<{ id: number, label: string, convention_date: Date, program_start: string, program_end: string } | null>}
 */
export async function getTodayConventionDay(year, todayIso) {
    const result = await exec(
        `SELECT TOP (1)
             id, label, convention_date, program_start, program_end
         FROM dbo.convention_days
         WHERE year            = @year
           AND convention_date = @date;`,
        (req) => {
            req.input('year', sql.Int, year);
            // Noon UTC avoids date-boundary shifts from the driver's
            // local-to-UTC conversion of a midnight Date object.
            req.input('date', sql.Date, new Date(todayIso.slice(0, 10) + 'T12:00:00Z'));
        }
    );
    return result.recordset?.[0] ?? null;
}
...
/**
 * Insert a single parking count record.
 * Called for 1-second heartbeats (isFinal = false, isManual = false),
 * tap-counter final submissions (isFinal = true, isManual = false),
 * and manually entered counts (isFinal = true, isManual = true).
 *
 * @param {{
 *   volunteerId:     number,
 ...
 * @returns {Promise<void>}
 */
export async function insertParkingCount(data) {
    await exec(
        `INSERT INTO dbo.parking_counts
             (volunteer_id, location_task_id, convention_day_id, count, is_final, is_manual, sub_location_id)
         VALUES
             (@volunteerId, @locationTaskId, @conventionDayId, @count, @isFinal, @isManual, @subLocationId);`,
        (req) => {
            req.input('volunteerId',     sql.Int, data.volunteerId);
            req.input('locationTaskId',  sql.Int, data.locationTaskId);
            req.input('conventionDayId', sql.Int, data.conventionDayId);
            req.input('count',           sql.Int, data.count);
            req.input('isFinal',         sql.Bit, data.isFinal  ? 1 : 0);
            req.input('isManual',        sql.Bit, data.isManual ? 1 : 0);
            req.input('subLocationId',   sql.Int, data.subLocationId ?? null);
        }
    );
}

/**
 * Fetch parking count data for the oversight time-series report.
 *
 * Records are bucketed into 15-minute intervals (floor to the nearest
 * quarter hour in UTC). Within each bucket, the peak count from each
 * volunteer is taken, then summed across volunteers per sub-location —
 * giving a granular count per entrance/floor/etc at each quarter-hour mark.
 * Counts without a sub_location_id are grouped under (unassigned).
 *
 * Note: bucket values are UTC DATETIME2. The frontend should convert
 * to Eastern Time before rendering chart axis labels.
 *
 * @param {number} dayId  convention_days.id
 * @returns {Promise<Array<{
 *   location_task_id:  number,
 ...
 * }>>}
 */
export async function getParkingCountReportData(dayId) {
    const result = await exec(
        `WITH bucketed AS (
             SELECT
                 pc.location_task_id,
                 ...
                 pc.volunteer_id,
                 DATEADD(MINUTE,
                     (DATEDIFF(MINUTE, '2000-01-01', pc.recorded_at) / 15) * 15,
                     '2000-01-01'
                 )                 AS bucket,
                 MAX(pc.count)     AS peak_count
             FROM  dbo.parking_counts pc
             WHERE pc.convention_day_id = @dayId
             GROUP BY
                 pc.location_task_id,
                 pc.sub_location_id,
                 pc.volunteer_id,
         ...
         SELECT
             ...
             svl.display_name      AS sub_type_name,
             b.bucket,
             SUM(b.peak_count)     AS total_count
         FROM  bucketed b
         JOIN  dbo.locations_tasks lt
             ON lt.id = b.location_task_id
         LEFT JOIN dbo.location_sub_locations lsl
             ON lsl.id = b.sub_location_id
         LEFT JOIN dbo.system_variable_lists svl
             ON svl.id = lsl.sub_type_id
         GROUP BY
             b.location_task_id, lt.name, lt.capacity,
             b.sub_location_id,  lsl.name, svl.display_name,
             b.bucket
         ORDER BY b.location_task_id, b.sub_location_id, b.bucket;`,
        (req) => {
            req.input('dayId', sql.Int, dayId);
        }
    );
...
/**
 * Delete all parking_counts records for a specific location on a given day.
 * Called by ASSISTANT_ADMIN+ to reset erroneous count data.
 * Hard-deletes — data cannot be recovered after this call.
 *
 * @param {number} locationTaskId   locations_tasks.id
 * @param {number} conventionDayId  convention_days.id
 * @returns {Promise<{ rowsDeleted: number }>}
 */
export async function deleteLocationCounts(locationTaskId, conventionDayId) {
    const result = await exec(
        `DELETE FROM dbo.parking_counts
         WHERE location_task_id  = @locationTaskId
           AND convention_day_id = @conventionDayId;`,
        (req) => {
            req.input('locationTaskId',  sql.Int, locationTaskId);
            req.input('conventionDayId', sql.Int, conventionDayId);
        }
    );
    return { rowsDeleted: result.rowsAffected?.[0] ?? 0 };
}
/**
 * Return active capacity alert rules for a location (and optional
 * sub-location), joined to the location's capacity so callers don't
 * need a second round-trip to compute percent-based thresholds.
 *
 * A rule with sub_location_id = NULL applies to the location as a whole;
 * a rule with sub_location_id set only applies to counts logged against
 * that specific sub-location.
 *
 * @param {number}      locationTaskId
 * @param {number|null} subLocationId
 * @returns {Promise<Array<{
 *   id:               number,
 *   location_task_id: number,
 *   sub_location_id:  number|null,
 ...
 * }>>}
 */
export async function getActiveCapacityAlertRulesForLocation(locationTaskId, subLocationId) {
    const result = await exec(
        `SELECT   r.id, r.location_task_id, r.sub_location_id, r.threshold_type,
                  r.threshold_value, r.direction, r.recipient_role,
                  r.message_override, r.is_armed, r.last_sent_at, lt.capacity
         FROM     dbo.capacity_alert_rules r
         JOIN     dbo.locations_tasks lt ON lt.id = r.location_task_id
         WHERE    r.active           = 1
           AND    r.location_task_id = @locationTaskId
           AND    (r.sub_location_id IS NULL OR r.sub_location_id = @subLocationId);`,
        (req) => {
            req.input('locationTaskId', sql.Int, locationTaskId);
            req.input('subLocationId', sql.Int, subLocationId ?? null);
        }
    );
    return (result.recordset || []).map((row) => ({
        ...row,
...
/**
 * Minimum time a capacity alert rule must wait before it can fire again,
 * independent of is_armed. is_armed is the primary edge-trigger guard,
 * but the 2026-07-03 incident (an undeployed fix plus corrupted count
 * data produced ~2.5 hours of continuous re-firing) showed the cost of
 * relying on a single mechanism. This cooldown is a second, independent
 * line of defense enforced atomically in the same UPDATE that claims
 * the rule -- see disarmCapacityAlertRule().
 */
const CAPACITY_ALERT_COOLDOWN_MINUTES = 5;

/**
 * Atomically claim a capacity alert rule for firing by flipping it from
 * armed to disarmed. Uses a conditional UPDATE (WHERE is_armed = 1 AND
 * the cooldown window has elapsed) so concurrent evaluations of the
 * same rule -- e.g. overlapping heartbeats while a prior evaluation is
 * still mid-send -- can only have one caller win the race. The loser
 * sees rowsAffected === 0 and must skip sending.
 *
 * The cooldown check is deliberately redundant with is_armed: even if
 * is_armed is somehow true again due to a future bug, this still blocks
 * a resend within CAPACITY_ALERT_COOLDOWN_MINUTES of the last send.
 *
 * @param {number} ruleId
 * @returns {Promise<boolean>} true if this call claimed the rule and should fire
 */
export async function disarmCapacityAlertRule(ruleId) {
    const result = await exec(
        `UPDATE dbo.capacity_alert_rules
         SET    is_armed     = 0,
                last_sent_at = SYSUTCDATETIME(),
                updated_at   = SYSUTCDATETIME()
         WHERE  id           = @ruleId
           AND  is_armed     = 1
           AND  (last_sent_at IS NULL OR last_sent_at <= DATEADD(MINUTE, -@cooldownMinutes, SYSUTCDATETIME()));`,
        (req) => {
            req.input('ruleId', sql.Int, ruleId);
            req.input('cooldownMinutes', sql.Int, CAPACITY_ALERT_COOLDOWN_MINUTES);
        }
    );
    return (result.rowsAffected?.[0] ?? 0) > 0;
}
...
/**
 * Re-arm a capacity alert rule (is_armed = 1) once the count has
 * returned to the safe side of its threshold.
 *
 * @param {number} ruleId
 * @returns {Promise<void>}
 */
export async function rearmCapacityAlertRule(ruleId) {
    await exec(
        `UPDATE dbo.capacity_alert_rules
         SET    is_armed   = 1,
                updated_at = SYSUTCDATETIME()
...
/**
 * Return active volunteers at or above a given role tier who have a
 * phone number on file. Used to resolve capacity alert recipients.
 *
 * @param {string[]} roles  e.g. ['OVERSEER', 'ASSISTANT_ADMIN', 'ADMIN']
 * @returns {Promise<Array<{ id: number, firstName: string, lastName: string, phone: string }>>}
 */
export async function getVolunteersByRoles(roles) {
    const result = await exec(
        `SELECT id, firstName, lastName, phone
         FROM   dbo.volunteer_in
         WHERE  role                 IN (${roles.map((_, i) => `@role${i}`).join(', ')})
           AND  active_current_year  = 1
           AND  registration_status  = 'completed'
...
/**
 * Log the outcome of a capacity alert send attempt.
 *
 * @param {{
 *   ruleId:          number,
 *   locationTaskId:  number,
 ...
 * @returns {Promise<void>}
 */
export async function logCapacityAlert(data) {
    await exec(
        `INSERT INTO dbo.capacity_alert_log
             (rule_id, location_task_id, triggered_count, recipient_count, status, error_msg)
         VALUES
             (@ruleId, @locationTaskId, @triggeredCount, @recipientCount, @status, @errorMsg);`,
        (req) => {
            req.input('ruleId', sql.Int, data.ruleId);
            req.input('locationTaskId', sql.Int, data.locationTaskId);
            req.input('triggeredCount', sql.Int, data.triggeredCount);
            req.input('recipientCount', sql.Int, data.recipientCount);
            req.input('status', sql.VarChar, data.status);
            req.input('errorMsg', sql.NVarChar, data.errorMsg ?? null);
        }
    );
}

/**
 * Fetch a single location_task row by id (name only — used to build
 * capacity alert message text without a second capacity query).
 *
 * @param {number} id
 * @returns {Promise<{ id: number, name: string } | null>}
 */
export async function getLocationTaskById(id) {
    const result = await exec(
        `SELECT id, name FROM dbo.locations_tasks WHERE id = @id;`,
        (req) => { req.input('id', sql.Int, id); }
    );
    return result.recordset?.[0] ?? null;
}
/**
 * Return all capacity alert rules across all locations, joined to
 * location and sub-location names for display on the management page.
 *
 * @returns {Promise<Array<{
 *   id:                number,
 *   location_task_id:  number,
 ...
 * }>>}
 */
export async function getAllCapacityAlertRules() {
    const result = await exec(
        `SELECT   r.id, r.location_task_id, lt.name AS location_name,
                  r.sub_location_id, lsl.name AS sub_location_name,
                  r.threshold_type, r.threshold_value, r.direction,
                  r.recipient_role, r.message_override, r.is_armed,
                  r.active, lt.capacity
         FROM     dbo.capacity_alert_rules r
         JOIN     dbo.locations_tasks lt ON lt.id = r.location_task_id
         LEFT JOIN dbo.location_sub_locations lsl ON lsl.id = r.sub_location_id
         ORDER BY lt.name, lsl.name;`
    );
    return (result.recordset || []).map((row) => ({
        ...row,
        is_armed: !!row.is_armed,
...
/**
 * Create a new capacity alert rule.
 *
 * @param {{
 *   locationTaskId:   number,
 ...
 */
export async function createCapacityAlertRule(data) {
    const result = await exec(
        `INSERT INTO dbo.capacity_alert_rules
             (location_task_id, sub_location_id, threshold_type, threshold_value,
              direction, recipient_role, message_override, created_by)
         OUTPUT INSERTED.id
         VALUES
             (@locationTaskId, @subLocationId, @thresholdType, @thresholdValue,
              @direction, @recipientRole, @messageOverride, @createdBy);`,
        (req) => {
            req.input('locationTaskId', sql.Int, data.locationTaskId);
            req.input('subLocationId', sql.Int, data.subLocationId ?? null);
            req.input('thresholdType', sql.VarChar, data.thresholdType);
            req.input('thresholdValue', sql.Int, data.thresholdValue);
            req.input('direction', sql.VarChar, data.direction);
            req.input('recipientRole', sql.VarChar, data.recipientRole);
            req.input('messageOverride', sql.NVarChar, data.messageOverride ?? null);
            req.input('createdBy', sql.Int, data.createdBy);
        }
    );
...
/**
 * Update an existing capacity alert rule. Editing a rule resets
 * is_armed to 1 so the new definition starts in a clean, fireable state.
 *
 * @param {number} id
 * @param {{
 *   locationTaskId:   number,
 ...
 * @returns {Promise<void>}
 */
export async function updateCapacityAlertRule(id, data) {
    await exec(
        `UPDATE dbo.capacity_alert_rules
         SET    location_task_id = @locationTaskId,
...
/**
 * Hard-delete a capacity alert rule. Its log rows remain for history
 * (rule_id becomes an orphaned reference — acceptable for an audit log).
 *
 * @param {number} id
 * @returns {Promise<void>}
 */
export async function deleteCapacityAlertRule(id) {
    await exec(
        `DELETE FROM dbo.capacity_alert_rules WHERE id = @id;`,
        (req) => { req.input('id', sql.Int, id); }
    );
}

/**
 * Bulk-set the active flag on multiple capacity alert rules at once, for
 * the management page's checkbox multi-select actions. Also resets
 * is_armed = 1 on every affected row, matching the same-rule behavior in
 * updateCapacityAlertRule() -- any config change re-arms the rule so it
 * doesn't come back active mid-crossing already disarmed.
 *
 * @param {number[]} ids
 * @param {boolean} active
 * @returns {Promise<void>}
 */
export async function bulkSetCapacityAlertRulesActive(ids, active) {
    if (!ids || ids.length === 0) return;
...
/**
 * Return the most recent capacity alert log entries, newest first, for
 * the management page's send history panel.
 *
 ...
 * }>>}
 */
export async function getCapacityAlertLog(limit = 100) {
    const result = await exec(
        `SELECT TOP (@limit)
                 l.id, l.rule_id, lt.name AS location_name, l.triggered_count,
                 l.recipient_count, l.status, l.error_msg, l.sent_at
         FROM     dbo.capacity_alert_log l
         JOIN     dbo.locations_tasks lt ON lt.id = l.location_task_id
...
/**
 * Read the capacity alert kill switch -- a single global flag that, when
 * enabled, stops evaluateCapacityAlerts() from sending ANY alert
 * regardless of individual rules' active/is_armed state. See
 * add_capacity_alert_kill_switch.sql for the motivating incident.
 *
 * @returns {Promise<{ enabled: boolean, enabledBy: number|null, enabledAt: Date|null }>}
 */
export async function getCapacityAlertKillSwitch() {
    const result = await exec(
        `SELECT kill_switch_enabled, kill_switch_enabled_by, kill_switch_enabled_at
         FROM   dbo.capacity_alert_settings
         WHERE  id = 1;`
    );
    const row = result.recordset?.[0];
    return {
        enabled: !!row?.kill_switch_enabled,
        enabledBy: row?.kill_switch_enabled_by ?? null,
        enabledAt: row?.kill_switch_enabled_at ?? null,
    };
}

/**
 * Set the capacity alert kill switch. See getCapacityAlertKillSwitch().
 *
 * @param {boolean} enabled
 * @param {number} userId  volunteer_in.id of whoever flipped it, for the audit trail
 * @returns {Promise<void>}
 */
export async function setCapacityAlertKillSwitch(enabled, userId) {
    await exec(
        `UPDATE dbo.capacity_alert_settings
         SET    kill_switch_enabled    = @enabled,
                kill_switch_enabled_by = @userId,
                kill_switch_enabled_at = SYSUTCDATETIME()
         WHERE  id = 1;`,
        (req) => {
...
/**
 * Return all active rows for a given category, with parent display_name.
 * Used to populate dropdowns (e.g. sub-location type pickers).
 *
 * @param {string} category  e.g. 'location_classification' | 'location_sub_type'
 * @returns {Promise<Array<{
 *   id:                  number,
 *   category:            string,
 *   display_name:        string,
 ...
 * }>>}
 */
export async function getSystemVariableList(category) {
    const result = await exec(
        `SELECT
             svl.id,
             svl.category,
             ...
             svl.display_order,
             svl.active
         FROM  dbo.system_variable_lists svl
         LEFT JOIN dbo.system_variable_lists p ON p.id = svl.parent_id
         WHERE svl.category = @category
         ORDER BY svl.display_order ASC, svl.display_name ASC;`,
        (req) => {
            req.input('category', sql.NVarChar(50), category);
        }
    );
...
/**
 * Return all rows across all categories, ordered by category then display_order.
 * Used by the System Variables management page.
 *
 * @returns {Promise<Array<{
 *   id:                  number,
 *   category:            string,
...
export async function getAllSystemVariables() {
    const result = await exec(
        `SELECT
         ...
         FROM  dbo.system_variable_lists svl
         LEFT JOIN dbo.system_variable_lists p ON p.id = svl.parent_id
         ORDER BY svl.category ASC, svl.display_order ASC, svl.display_name ASC;`,
        () => {}
    );
    return result.recordset || [];
}
...
/**
 * Insert a new system variable list entry.
 ...
 * @returns {Promise<number>}  new row id
 */
export async function createSystemVariable(data) {
    const result = await exec(
        `INSERT INTO dbo.system_variable_lists
             (category, display_name, parent_id, display_order)
         OUTPUT INSERTED.id
         VALUES
             (@category, @displayName, @parentId, @displayOrder);`,
        (req) => {
            req.input('category',     sql.NVarChar(50),  data.category);
            req.input('displayName',  sql.NVarChar(100), data.displayName.trim());
            req.input('parentId',     sql.Int,            data.parentId ?? null);
            req.input('displayOrder', sql.Int,            data.displayOrder ?? 0);
        }
    );
    const id = result.recordset?.[0]?.id;
    if (!id) throw new Error('INSERT system_variable_lists did not return id.');
    return id;
}

/**
 * Update an existing system variable list entry.
 *
 * @param {number} id
 * @param {{
 *   displayName:   string,
 ...
 * @returns {Promise<boolean>}  true if a row was updated
 */
export async function updateSystemVariable(id, data) {
    const result = await exec(
        `UPDATE dbo.system_variable_lists
...
/**
 * Check whether a system variable is referenced before deleting it.
 * Returns reference counts; the caller decides whether to block the delete.
 *
 * @param {number} id
 * @returns {Promise<{ locCount: number, subCount: number, childCount: number }>}
 */
export async function checkSystemVariableRefs(id) {
    const result = await exec(
        `SELECT
             (SELECT COUNT(*) FROM dbo.locations_tasks
              WHERE  classification_id = @id)                      AS loc_count,
             (SELECT COUNT(*) FROM dbo.location_sub_locations
              WHERE  sub_type_id       = @id)                      AS sub_count,
             (SELECT COUNT(*) FROM dbo.system_variable_lists
              WHERE  parent_id = @id AND active = 1)               AS child_count;`,
        (req) => { req.input('id', sql.Int, id); }
    );
    const row = result.recordset?.[0] ?? {};
    return {
        locCount:   row.loc_count   ?? 0,
        subCount:   row.sub_count   ?? 0,
        childCount: row.child_count ?? 0,
    };
}

/**
 * Hard-delete a system variable list entry.
 * Caller must call checkSystemVariableRefs first and abort if any refs exist.
 *
 * @param {number} id
 * @returns {Promise<boolean>}
 */
export async function deleteSystemVariable(id) {
    const result = await exec(
        `DELETE FROM dbo.system_variable_lists WHERE id = @id;`,
        (req) => { req.input('id', sql.Int, id); }
    );
    const affected = Array.isArray(result.rowsAffected)
        ? result.rowsAffected.reduce((s, n) => s + n, 0)
 ...
 *
 * @param {number}      locationTaskId
 * @param {number|null} classificationId  null to clear
 * @returns {Promise<boolean>}
 */
export async function setLocationClassification(locationTaskId, classificationId) {
    const result = await exec(
        `UPDATE dbo.locations_tasks
        ...
        (req) => {
            req.input('locationTaskId',   sql.Int, locationTaskId);
            req.input('classificationId', sql.Int, classificationId ?? null);
        }
    );
    const affected = Array.isArray(result.rowsAffected)
        ? result.rowsAffected.reduce((s, n) => s + n, 0)
...
/**
 * Return all sub-locations for a location, with sub-type display name.
 * Includes both active and inactive rows so the management UI can show all.
 *
 * @param {number} locationTaskId
 * @returns {Promise<Array<{
 *   id:             number,
 ...
 * }>>}
 */
export async function getSubLocationsForLocation(locationTaskId) {
    const result = await exec(
        `SELECT
             lsl.id,
             lsl.location_task_id,
             lsl.name,
             lsl.sub_type_id,
             svl.display_name AS sub_type_name,
             lsl.display_order,
             lsl.active
         FROM  dbo.location_sub_locations lsl
         LEFT JOIN dbo.system_variable_lists svl ON svl.id = lsl.sub_type_id
         WHERE lsl.location_task_id = @locationTaskId
         ORDER BY lsl.display_order ASC, lsl.name ASC;`,
        (req) => {
            req.input('locationTaskId', sql.Int, locationTaskId);
        }
    );
...
/**
 * Return only active sub-locations for the tally-page picker.
 *
 * @param {number} locationTaskId
 * @returns {Promise<Array<{ id: number, name: string, sub_type_name: string | null }>>}
 */
export async function getActiveSubLocationsForLocation(locationTaskId) {
    const result = await exec(
        `SELECT
             lsl.id,
             lsl.name,
 ...
 * @returns {Promise<number>}  new row id
 */
export async function createSubLocation(data) {
    const result = await exec(
        `INSERT INTO dbo.location_sub_locations
             (location_task_id, name, sub_type_id, display_order)
         OUTPUT INSERTED.id
         VALUES
             (@locationTaskId, @name, @subTypeId, @displayOrder);`,
        (req) => {
            req.input('locationTaskId', sql.Int,           data.locationTaskId);
            req.input('name',           sql.NVarChar(100), data.name.trim());
            req.input('subTypeId',      sql.Int,           data.subTypeId    ?? null);
            req.input('displayOrder',   sql.Int,           data.displayOrder ?? 0);
        }
    );
    const id = result.recordset?.[0]?.id;
    if (!id) throw new Error('INSERT location_sub_locations did not return id.');
    return id;
}

/**
 * Update an existing sub-location.
 *
 * @param {number} id
 * @param {{
 *   name:          string,
 ...
 * @returns {Promise<boolean>}
 */
export async function updateSubLocation(id, data) {
    const result = await exec(
        `UPDATE dbo.location_sub_locations
         SET name          = @name,
 ...
 * parking_counts rows that referenced it will have sub_location_id set to NULL
 * via the ON DELETE SET NULL FK constraint — count data is preserved.
 *
 * @param {number} id
 * @returns {Promise<boolean>}
 */
export async function deleteSubLocation(id) {
    const result = await exec(
        `DELETE FROM dbo.location_sub_locations WHERE id = @id;`,
        (req) => { req.input('id', sql.Int, id); }
    );
    const affected = Array.isArray(result.rowsAffected)
        ? result.rowsAffected.reduce((s, n) => s + n, 0)
 ...
 * orderedIds is the desired sequence; index 0 gets display_order 0, etc.
 * Only updates rows belonging to the given location (prevents cross-location tampering).
 *
 * @param {number}   locationTaskId
 * @param {number[]} orderedIds  sub-location IDs in the desired order
 * @returns {Promise<void>}
 */
export async function reorderSubLocations(locationTaskId, orderedIds) {
    for (let i = 0; i < orderedIds.length; i++) {
        await exec(
            `UPDATE dbo.location_sub_locations
             SET display_order = @order
             WHERE id = @id AND location_task_id = @locationTaskId;`,
            (req) => {
                req.input('id',              sql.Int, orderedIds[i]);
                req.input('order',           sql.Int, i);
                req.input('locationTaskId',  sql.Int, locationTaskId);
            }
        );
    }
...
/**
 * Return distinct convention years available for the year selector,
 * derived from convention_days, ordered descending.
 *
 * @returns {Promise<number[]>}
 */
export async function getLessonsLearnedYears() {
    const result = await exec(
        `SELECT DISTINCT YEAR(cd.convention_date) AS year
         FROM   dbo.convention_days cd
         WHERE  cd.convention_date IS NOT NULL
         ORDER  BY year DESC;`,
        () => {}
    );
    return (result.recordset || []).map((r) => r.year);
}

/**
 * Return active lesson-department entries from system_variable_lists,
 * ordered by display_order then display_name.
 *
 * @returns {Promise<Array<{ id: number, display_name: string }>>}
 */
export async function getLessonsLearnedDepartments() {
    const result = await exec(
...
/**
 * Return lessons optionally filtered by year and status.
 * status can be a single value ('published') or comma-separated
 * ('submitted,approved') — the function splits and uses IN.
 *
 * @param {{
 *   year?:   number | null,
 *   status?: string | null,
 ...
 * @returns {Promise<object[]>}
 */
export async function getLessonsLearned({ year = null, status = null, archivedOnly = false, audience = null } = {}) {
    const statusList = status
        ? status.split(',').map((s) => `'${s.trim().replace(/'/g, '')}'`).join(', ')
        : null;
    ...
    // Flags are not complementary — a both-flagged lesson appears under either
    // filter, so these are inclusive tests rather than an either/or split.
    const audienceFilter = audience === 'internal'  ? ' AND ll.is_internal  = 1'
                         : audience === 'committee' ? ' AND ll.is_committee = 1'
                         : '';
    ...
    const result = await exec(
        `SELECT
             ...
             ll.year,
             ll.department_id,
             svl.display_name                      AS department_name,
             ll.department_other,
             ll.notes,
             ll.overseer_comments,
             ll.status,
             ll.archived,
             ll.archived_by,
             arb.firstName + ' ' + arb.lastName    AS archived_by_name,
             ll.archived_at,
             ll.submitted_by,
             sb.firstName + ' ' + sb.lastName    AS submitted_by_name,
             ll.submitted_at,
             ll.approved_by,
             ab.firstName + ' ' + ab.lastName    AS approved_by_name,
             ll.approved_at,
             ll.published_by,
             pb.firstName + ' ' + pb.lastName    AS published_by_name,
             ll.published_at,
             ll.is_internal,
             ll.is_committee,
             (SELECT COUNT(*) FROM dbo.lessons_learned_photos llp
              WHERE llp.lesson_id = ll.id)          AS photo_count
         FROM  dbo.lessons_learned ll
         JOIN  dbo.volunteer_in sb  ON sb.id = ll.submitted_by
         LEFT JOIN dbo.system_variable_lists svl ON svl.id = ll.department_id
         LEFT JOIN dbo.volunteer_in ab  ON ab.id = ll.approved_by
         LEFT JOIN dbo.volunteer_in pb  ON pb.id = ll.published_by
         LEFT JOIN dbo.volunteer_in arb ON arb.id = ll.archived_by
         WHERE (@year   IS NULL OR ll.year = @year)
           AND ${archivedFilter}
           AND (${statusList ? `ll.status IN (${statusList})` : '1=1'})${audienceFilter}
         ORDER BY ll.submitted_at DESC;`,
        (req) => {
            req.input('year', sql.Int, year ?? null);
        }
    );
    return result.recordset || [];
}
...
/**
 * Return a single lesson by id with all join columns.
 *
 * @param {number} id
 * @returns {Promise<object | null>}
 */
 ...
 * The lesson's status (submitted/approved/published) is preserved intact;
 * only the archived flag and audit fields change.
 *
 * @param {number}  id
 ...
 * @returns {Promise<void>}
 */
export async function setLessonArchived(id, archived, userId) {
    await exec(
        `UPDATE dbo.lessons_learned
         SET archived    = @archived,
             archived_by = CASE WHEN @archived = 1 THEN @userId ELSE NULL END,
             archived_at = CASE WHEN @archived = 1 THEN SYSUTCDATETIME() ELSE NULL END
         WHERE id = @id;`,
        (req) => {
            req.input('id',       sql.Int, id);
            req.input('archived', sql.Bit, archived ? 1 : 0);
            req.input('userId',   sql.Int, userId);
        }
    );
}
...
/**
 * Set the audience flags for a lesson.
 *
 * At least one flag must be true — CK_ll_audience rejects the update
 * otherwise, so callers should validate before calling to return a useful
 * message rather than a constraint violation.
 *
 * @param {number}  id
 * @param {boolean} isInternal   Internal to the department.
 * @param {boolean} isCommittee  Goes to the committee with the operating plan.
 * @returns {Promise<void>}
 */
export async function setLessonFlags(id, isInternal, isCommittee) {
    await exec(
        `UPDATE dbo.lessons_learned
        ...
        (req) => {
            req.input('id',          sql.Int, id);
            req.input('isInternal',  sql.Bit, isInternal  ? 1 : 0);
            req.input('isCommittee', sql.Bit, isCommittee ? 1 : 0);
        }
    );
}

/**
 * Move a lesson backwards in the workflow, clearing the audit stamps for every
 * stage at or above the target so the record cannot claim an approval or
 * publication it no longer holds.
 *
 *   published → approved   clears published_by / published_at
 *   approved  → submitted  clears approved_by / approved_at and the published pair
 *
 * The original approver is preserved when reverting to 'approved' — only the
 * publication is undone.
 *
 * @param {number} id
 * @param {'approved'|'submitted'} status  Stage to revert to.
 * @returns {Promise<void>}
 */
export async function revertLessonStatus(id, status) {
    const clearApproved = status === 'submitted'
...
/**
 * Return all photo records for a lesson, ordered by upload time.
 *
 * @param {number} lessonId
 * @returns {Promise<Array<{ id: number, blob_name: string, original_filename: string, uploaded_at: Date }>>}
 */
export async function getLessonPhotos(lessonId) {
    const result = await exec(
        `SELECT id, lesson_id, blob_name, original_filename, uploaded_by, uploaded_at
         FROM   dbo.lessons_learned_photos
         WHERE  lesson_id = @lessonId
         ORDER  BY uploaded_at ASC;`,
        (req) => { req.input('lessonId', sql.Int, lessonId); }
    );
    return result.recordset || [];
}
 ...
 *   submittedBy:      number,
 * }} data
 * @returns {Promise<number>} New lesson id.
 */
export async function createLesson(data) {
    const result = await exec(
        `INSERT INTO dbo.lessons_learned
             (year, department_id, department_other, notes, submitted_by)
         OUTPUT INSERTED.id
         VALUES
             (@year, @departmentId, @departmentOther, @notes, @submittedBy);`,
        (req) => {
            req.input('year',            sql.Int,               data.year);
            req.input('departmentId',    sql.Int,               data.departmentId    ?? null);
            req.input('departmentOther', sql.NVarChar(100),     data.departmentOther ?? null);
            req.input('notes',           sql.NVarChar(sql.MAX), data.notes);
            req.input('submittedBy',     sql.Int,               data.submittedBy);
        }
    );
    const id = result.recordset?.[0]?.id;
    if (!id) throw new Error('INSERT lessons_learned did not return id.');
    return id;
}

/**
 * Update editable fields on a lesson.
 *
 * @param {number} id
 * @param {{
 *   year:             number,
    ...
    await exec(
        `UPDATE dbo.lessons_learned
         SET year              = @year,
             ...
             notes             = @notes,
             overseer_comments = @overseerComments
         WHERE id = @id;`,
        (req) => {
            req.input('id',               sql.Int,               id);
            req.input('year',             sql.Int,               data.year);
            ...
            req.input('departmentOther',  sql.NVarChar(100),     data.departmentOther ?? null);
            req.input('notes',            sql.NVarChar(sql.MAX), data.notes);
            req.input('overseerComments', sql.NVarChar(sql.MAX), data.overseerComments ?? null);
        }
    );
}

/**
 * Advance a lesson to 'approved' or 'published' status, recording who
 * performed the action and when.
 *
 * @param {number} id
 * @param {'approved' | 'published'} status
 * @param {number} userId
 * @returns {Promise<void>}
 */
export async function setLessonStatus(id, status, userId) {
    const approvedFields  = status === 'approved'  ? `approved_by  = @userId, approved_at  = SYSUTCDATETIME(),` : '';
    const publishedFields = status === 'published' ? `published_by = @userId, published_at = SYSUTCDATETIME(),` : '';
    await exec(
        `UPDATE dbo.lessons_learned
         SET status = @status,
             ${approvedFields}
...
/**
 * Promote a set of approved lessons to published in a single statement.
 *
 * Only rows that are still 'approved', unarchived, and in the given year are
 * touched, so a stale selection in the browser can never republish a lesson or
 * pull one in from another year. The id array travels as JSON and is expanded
 * with OPENJSON — a TVP would require a CREATE TYPE, which this schema avoids.
 *
 * @param {number[]} ids     Lesson ids to publish.
 * @param {number}   year    Convention year the ids must belong to.
 * @param {number}   userId  Volunteer id recorded as published_by.
 * @returns {Promise<number>} Count of rows actually updated.
 */
export async function publishLessonsBulk(ids, year, userId) {
    if (!Array.isArray(ids) || ids.length === 0) return 0;

    const result = await exec(
        `UPDATE ll
         SET    ll.status       = 'published',
                ll.published_by = @userId,
                ll.published_at = SYSUTCDATETIME()
         FROM   dbo.lessons_learned ll
         JOIN   OPENJSON(@ids) WITH (id INT '$') j ON j.id = ll.id
         WHERE  ll.status   = 'approved'
           AND  ll.archived = 0
           AND  ll.year     = @year;`,
        (req) => {
            req.input('ids',    sql.NVarChar(sql.MAX), JSON.stringify(ids));
            req.input('year',   sql.Int,               year);
            req.input('userId', sql.Int,               userId);
        }
    );
...
/**
 * Insert a photo attachment record for a lesson.
 *
 * @param {number} lessonId
 * @param {string} blobName
 * @param {string} originalFilename
 * @param {number} uploadedBy
 * @returns {Promise<number>} New photo record id.
 */
export async function addLessonPhotoRecord(lessonId, blobName, originalFilename, uploadedBy) {
    const result = await exec(
        `INSERT INTO dbo.lessons_learned_photos
             (lesson_id, blob_name, original_filename, uploaded_by)
         OUTPUT INSERTED.id
         VALUES
             (@lessonId, @blobName, @originalFilename, @uploadedBy);`,
        (req) => {
            req.input('lessonId',         sql.Int,           lessonId);
            req.input('blobName',         sql.NVarChar(500), blobName);
            req.input('originalFilename', sql.NVarChar(260), originalFilename);
            req.input('uploadedBy',       sql.Int,           uploadedBy);
        }
    );
    const id = result.recordset?.[0]?.id;
    if (!id) throw new Error('INSERT lessons_learned_photos did not return id.');
    return id;
}

/**
 * Delete a photo attachment record by id.
 * ON DELETE CASCADE on lessons_learned_photos handles cascaded deletes
 * when a lesson is deleted; this function handles individual photo removal.
 *
 * @param {number} photoId
 * @returns {Promise<boolean>}
 */
export async function deleteLessonPhotoRecord(photoId) {
    const result = await exec(
        `DELETE FROM dbo.lessons_learned_photos WHERE id = @id;`,
        (req) => { req.input('id', sql.Int, photoId); }
    );
    const affected = Array.isArray(result.rowsAffected)
        ? result.rowsAffected.reduce((s, n) => s + n, 0)
        : result.rowsAffected || 0;
...
/**
 * Permanently delete a lesson row.
 *
 * Photo rows in lessons_learned_photos are removed by the ON DELETE CASCADE on
 * FK_llp_lesson. Callers MUST delete the underlying blobs BEFORE calling this —
 * once the photo rows are gone the blob names are unrecoverable and the files
 * orphan in the lessons-learned container with no way to identify them.
 *
 * @param {number} id
 * @returns {Promise<boolean>} True if a row was deleted.
 */
export async function deleteLesson(id) {
    const result = await exec(
        `DELETE FROM dbo.lessons_learned WHERE id = @id;`,
        (req) => { req.input('id', sql.Int, id); }
    );
    const affected = Array.isArray(result.rowsAffected)
        ? result.rowsAffected.reduce((s, n) => s + n, 0)
...
/**
 * Upsert the published report record for a convention year.
 * Called after PDF generation succeeds; one row per year (year is the PK).
 *
 * @param {number} year
 * @param {string} blobName
 ...
 * @returns {Promise<void>}
 */
export async function upsertLessonsLearnedReport(year, blobName, shareUrl, publishedBy) {
    await exec(
        `MERGE dbo.lessons_learned_reports AS target
         USING (SELECT @year AS year) AS source ON target.year = source.year
         WHEN MATCHED THEN
             UPDATE SET blob_name    = @blobName,
                        share_url    = @shareUrl,
                        published_by = @publishedBy,
                        published_at = SYSUTCDATETIME()
         WHEN NOT MATCHED THEN
             INSERT (year, blob_name, share_url, published_by)
             VALUES (@year, @blobName, @shareUrl, @publishedBy);`,
        (req) => {
            req.input('year',        sql.Int,            year);
            req.input('blobName',    sql.NVarChar(500),  blobName);
            req.input('shareUrl',    sql.NVarChar(2000), shareUrl);
            req.input('publishedBy', sql.Int,            publishedBy);
        }
    );
}

/**
 * Return the published report record for a convention year, or null if none.
 * Includes a count of published lessons for that year.
 *
 * @param {number} year
 * @returns {Promise<{
 *   year:               number,
 ...
 * } | null>}
 */
export async function getLessonsLearnedReport(year) {
    const result = await exec(
        `SELECT llr.year, llr.blob_name, llr.share_url, llr.published_by,
                llr.published_at,
                v.firstName + ' ' + v.lastName AS published_by_name,
                (SELECT COUNT(*) FROM dbo.lessons_learned ll
                 WHERE  ll.year = llr.year AND ll.status = 'published') AS lesson_count
         FROM   dbo.lessons_learned_reports llr
         JOIN   dbo.volunteer_in v ON v.id = llr.published_by
         WHERE  llr.year = @year;`,
        (req) => { req.input('year', sql.Int, year); }
    );
    return result.recordset?.[0] || null;
