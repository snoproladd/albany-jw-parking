 ...
 *  - Enter free text and have AI interpret it into a new suggestion.
 *
 * Public API:
...
function _renderBody(body, volId) {
  ...
  // ── Overseer interpret form ───────────────────────────────────────
  const interpSection = _buildInterpretForm(volId, body, sugSection);
  body.appendChild(interpSection);
...
/**
 * Builds the overseer interpret form section.
 *
 * @param {number}      volId
 * @param {HTMLElement} body       - Panel body for re-positioning after content change.
 * @param {HTMLElement} sugSection - Suggestion list section to prepend new card to.
 * @returns {HTMLElement}
 */
function _buildInterpretForm(volId, body, sugSection) {
  const section = document.createElement("div");
  section.classList.add("sched-note-section");

  const label = document.createElement("div");
  label.classList.add("sched-note-label");
  label.innerHTML =
    '<i class="fa-solid fa-wand-magic-sparkles me-1"></i>Interpret Constraint';
  section.appendChild(label);

  const hint = document.createElement("p");
  hint.classList.add("sched-note-meta");
  hint.textContent =
    "Describe a constraint the AI couldn't determine, e.g. \"He needs to leave by noon Saturday for his son's baptism.\"";
  section.appendChild(hint);

  const textarea = document.createElement("textarea");
  textarea.className = "sched-constraint-interpret-input";
  textarea.rows = 2;
  textarea.placeholder = "Describe the constraint…";
  ...
  preview.classList.add("sched-constraint-interpret-preview", "d-none");
  section.appendChild(preview);

  // Buttons
  const btnRow = document.createElement("div");
  btnRow.classList.add("sched-constraint-btn-row");

  const interpretBtn = document.createElement("button");
  interpretBtn.type = "button";
  interpretBtn.className = "sched-constraint-interpret-btn";
  interpretBtn.innerHTML =
    '<i class="fa-solid fa-wand-magic-sparkles me-1"></i>Interpret';
  ...
  btnRow.appendChild(interpretBtn);
...
/**
 * Handles the Interpret button click.
 *
 ...
 * @param {number} volId
 * @param {HTMLElement} body
 * @param {HTMLElement} sugSection
 * @returns {Promise<void>}
 */
async function _onInterpret(
  btn,
  textarea,
  errEl,
  preview,
  ...
  if (!text) {
    errEl.textContent = "Please enter a constraint description.";
    errEl.classList.remove("d-none");
    return;
  }
  ...
  btn.disabled = true;
  btn.innerHTML =
    '<span class="spinner-border spinner-border-sm me-1"></span>Interpreting…';

  try {
    const res = await fetch(`/api/constraints/${volId}/interpret`, {
      method: "POST",
    ...
    if (!res.ok || !data.suggestion) {
      errEl.textContent =
        data.error || "Interpretation failed. Please try again.";
      errEl.classList.remove("d-none");
      btn.disabled = false;
      btn.innerHTML =
        '<i class="fa-solid fa-wand-magic-sparkles me-1"></i>Interpret';
    ...
    preview.innerHTML = _buildInterpretPreviewHtml(sug);

    // Wire Save Suggestion button inside preview
    const saveBtn = preview.querySelector(".sched-constraint-save-btn");
    if (saveBtn) {
      const csrf =
      ...
      saveBtn.addEventListener("click", async () => {
        ...
        try {
          const saveRes = await fetch(`/api/constraints/${volId}/suggestions`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              blackoutType: sug.blackoutType,
              description: sug.description,
              dayHint: sug.dayHint,
              timeHint: sug.timeHint,
              conventionDayId: sug.conventionDayId,
              startMins: sug.startMins,
              endMins: sug.endMins,
            }),
          });
          ...
          // Add to in-memory list and render a new card
          const newSuggestion = {
            id: saved.id,
            source_type: "overseer",
            source_id: null,
            blackout_type: sug.blackoutType || "Custom",
            description: sug.description || text,
            day_hint: sug.dayHint,
            time_hint: sug.timeHint,
            convention_day_id: sug.conventionDayId,
            day_label:
              _days.find((d) => d.id === sug.conventionDayId)?.label || null,
            start_mins: sug.startMins,
            end_mins: sug.endMins,
            created_at: new Date().toISOString(),
          };
          _suggestions.unshift(newSuggestion);
          _updatePillBadge(volId, _suggestions.length);

          // Prepend new card to suggestion section
          const noConstraintsMsg = sugSection.querySelector(".sched-note-meta");
          if (noConstraintsMsg) noConstraintsMsg.remove();
          const card = _buildSuggestionCard(newSuggestion, volId);
          const firstCard = sugSection.querySelector(".sched-constraint-card");
          sugSection.insertBefore(card, firstCard || null);
          ...
          saveBtn.innerHTML =
            '<i class="fa-solid fa-floppy-disk me-1"></i>Save Suggestion';
        }
      });
    }
...
/**
 * Builds the HTML for the interpret result preview.
 *
 * @param {object} sug - Suggestion from the interpret API.
 * @returns {string}
 */
function _buildInterpretPreviewHtml(sug) {
  const timeStr =
    sug.startMins != null && sug.endMins != null
      ? `${_fmtMins(sug.startMins)} – ${_fmtMins(sug.endMins)}`
      : "Time not resolved — enter manually before applying.";

  return `
        <div class="sched-constraint-interpret-result">
            <div class="sched-note-label mb-1">
                <i class="fa-solid fa-wand-magic-sparkles me-1"></i>AI Result
            </div>
            <div class="sched-constraint-desc">${_esc(sug.description || "—")}</div>
            <div class="sched-constraint-resolved mt-1">
                <strong>${_esc(sug.dayHint || "Day not determined")}</strong>
                &nbsp;·&nbsp; ${_esc(timeStr)}
                &nbsp;<span class="sched-constraint-type-badge">${_esc(sug.blackoutType || "Custom")}</span>
            </div>
            <div class="sched-constraint-btn-row mt-2">
                <button type="button" class="sched-constraint-save-btn">
                    <i class="fa-solid fa-floppy-disk me-1"></i>Save Suggestion
                </button>
            </div>
        </div>
