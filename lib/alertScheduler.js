...
async function tick(deps) {
  ...
  for (const schedule of schedules) {
    ...
    const fireUtc = new Date(`${fireDateStr}T${schedule.fire_time_utc}Z`);
    const msAgo = now - fireUtc;

    // Only fire if scheduled time is within the past BURST_WINDOW_MINUTES
    if (msAgo < 0 || msAgo > burstWindowMs) continue;
  ...
  // ── 2. T-15 rolling ─────────────────────────────────────────────────────
  try {
    const candidates = await getT15CandidateShifts(year, today);

    const t15Rows = candidates.filter((row) => {
      const shiftUtc = shiftToUtc(row.convention_date, row.start_time);
      const minutesUntil = (shiftUtc - now) / (60 * 1000);
      return minutesUntil >= T15_MIN_MINUTES && minutesUntil <= T15_MAX_MINUTES;
    });
