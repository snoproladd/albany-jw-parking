...
export async function insertDraftNameEmail(firstName, lastName, suffix, email) {
  ...
  console.log("[dbSync.insertDraftNameEmail] inserting new draft:", {
    firstName,
    lastName,
    suffix,
    email,
  ...
  console.log("[dbSync.insertDraftNameEmail] inserted row:", row);
  return row;
}
...
export async function updateDraftNameEmail(
  ...
  console.log("[dbSync.updateDraftNameEmail] registrationId:", registrationId, {
    firstName,
    lastName,
    suffix,
    email,
  ...
  const row = result.recordset?.[0] ?? null;
  console.log("[dbSync.updateDraftNameEmail] recordset:", row);
  return row;
}
 ...
 *
 * Sets registration_status = 'completed' for the given registration_id.
 *
...
export async function markDraftCompleted(registrationId) {
  ...
  const result = await exec(sqlText, (req) => {
    // registration_id is UNIQUEIDENTIFIER in your other code,
    // so keep the SQL type consistent:
    req.input("registrationId", sql.UniqueIdentifier, registrationId);
  });
