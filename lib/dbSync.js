 ...
 * event_type_id and related fields are LEFT JOINed — meeting shifts
 * (is_meeting = 1) have no event type and return NULL for those columns.
 *
 * @param {number} sessionId
 * @returns {Promise<Array<{id:number, session_id:number,
 *   event_type_id:number|null, event_type_name:string|null,
 *   event_type_color:string|null, label:string, start_time:string,
 *   end_time:string, volunteer_need:number|null, notes:string|null,
 *   invitable:boolean, sms_code:string|null, department:string|null,
 *   is_meeting:boolean}>>}
 */
  ...
  const result = await exec(
    `
        SELECT
            ...
            s.sms_code, s.department, s.is_meeting
        FROM dbo.shifts s
        LEFT JOIN dbo.event_types et ON et.id = s.event_type_id
 ...
 * Create a shift.
 *
 * @param {{session_id:number, event_type_id?:number|null, label:string,
  ...
  const result = await exec(
    `
        INSERT INTO dbo.shifts
            (session_id, event_type_id, label, start_time, end_time,
             department, volunteer_need, notes, sms_code, is_meeting)
        OUTPUT INSERTED.id
        VALUES (@sessionId, @eventTypeId, @label, @start_time, @end_time,
                @department, @volunteer_need, @notes, @smsCode, @isMeeting);
    `,
    (req) => {
      req.input("sessionId",      sql.Int,          data.session_id);
      req.input("eventTypeId",    sql.Int,          data.event_type_id ?? null);
      req.input("label",          sql.NVarChar(50), data.label.trim());
      req.input("start_time",     sql.NVarChar(8),  data.start_time);
      req.input("end_time",       sql.NVarChar(8),  data.end_time);
      req.input("department",     sql.NVarChar(50), data.department?.trim() || null);
 ...
 *
 * @param {number} id
 * @param {{event_type_id?:number|null, label:string, start_time:string,
 *   end_time:string, department?:string|null, volunteer_need?:number|null,
 *   notes?:string|null, sms_code?:string|null, invitable?:boolean,
 *   is_meeting?:boolean}} data
 * @returns {Promise<boolean>}
 */
 ...
 *
 * Department map (matches dbo.shifts.department column values):
 ...
 * @param {string|null} department      - dbo.shifts.department value (null for meetings)
 * @param {number}      sequenceNumber  - 1-based position within the group
 * @param {boolean}     [isMeeting]     - Forces MT dept code; ignores department
 * @returns {string} Uppercase code, e.g. "FRLG1", "FRMT1"
 */
export function generateShiftCode(conventionDate, department, sequenceNumber, isMeeting = false) {
...
export async function getRendezvousForDay(dayId) {
    const result = await exec(`
        ...
        ORDER BY sh.start_time, et.name, lt.name;
    `, (req) => {
        ...
        sourceShifts.map(async (shift) => {
          ...
          const [newShiftId, sourceAssignments] = await Promise.all([
            createShift({
              ...
              event_type_id:  shift.event_type_id ?? null,
              label:          shift.label,
...
export async function getSchedulerReportData(dayId) {
    ...
    const DEPT_ORDER = ['lots_and_garages', 'signs', 'security', 'dropoff_pickup', 'mobile_support', 'desk'];
...
export async function getSchedulerData(dayId) {
    const result = await exec(`
        ...
        WHERE cd.id = @dayId
          AND (sh.department IS NOT NULL OR sh.is_meeting = 1)
...
export async function getCrewStaffingSummary(year) {
    ...
    const scheduledMap = {};
    for (const r of scheduledResult.recordset || []) {
        scheduledMap[r.department] = Number(r.scheduled_count) || 0;
    }
...
export async function getDayStaffingReport(dayId) {
    ...
    const DEPT_ORDER = [
        'lots_and_garages', 'signs', 'security',
        'dropoff_pickup', 'mobile_support', 'desk',
    ];
    ...
    const result = await exec(
        `SELECT
         ...
         WHERE se.convention_day_id = @dayId
           AND sh.department        IS NOT NULL
           AND sh.department        <> ''
         ORDER BY sh.department, sh.start_time;`,
        (req) => { req.input('dayId', sql.Int, dayId); },
    );
    ...
    for (const r of rows) {
        ...
        if (!deptMap.has(key)) {
            deptMap.set(key, { volunteer_need: 0, scheduled: 0, attended: 0 });
        }
    ...
    return DEPT_ORDER
        .filter((k) => deptMap.has(k))
        .map((k) => {
            const d = deptMap.get(k);
            return {
                department:     k,
                label:          DEPT_LABELS[k] || k,
