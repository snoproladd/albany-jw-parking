...
/**
 * Get a volunteer's saved scheduler department filter preference.
 *
 * @param {number} volunteerId
 * @returns {Promise<string|null>}
 */
export async function getSchedulerDeptFilter(volunteerId) {
    const result = await exec(`
        SELECT scheduler_dept_filter
        FROM dbo.volunteer_in
        WHERE id = @id;
    `, (req) => {
        req.input('id', sql.Int, volunteerId);
    });
    return result.recordset[0]?.scheduler_dept_filter || null;
}

/**
 * Save a volunteer's scheduler department filter preference.
 * An empty string clears the preference back to "All Departments".
 *
 * @param {number} volunteerId
 * @param {string} value  - One of the department keys, or '' to clear.
 * @returns {Promise<boolean>} True if a row was updated.
 */
export async function updateSchedulerDeptFilter(volunteerId, value) {
    const result = await exec(`
        UPDATE dbo.volunteer_in
        SET scheduler_dept_filter = @value
        WHERE id = @id;
    `, (req) => {
        req.input('id', sql.Int, volunteerId);
        req.input('value', sql.NVarChar(30), value || null);
    });
    const affected = Array.isArray(result.rowsAffected)
        ? result.rowsAffected.reduce((s, n) => s + n, 0)
        : result.rowsAffected || 0;
