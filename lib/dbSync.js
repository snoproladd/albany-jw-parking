 ...
 *   → schedule_assignments (cascades shift_rendezvous_points)
 *   → shifts → sessions
 *
 * @param {number} id - Session primary key.
 * @returns {Promise<boolean>} True if the session row was deleted.
 */
 ...
 *   unknown/null     → XX
 *
 * The sequence number is always present — the caller is responsible for
 * supplying it. For live UI suggestions use the /api/shifts/suggest-code
 * endpoint which queries the DB to derive the next available number.
 * For bulk backfills use the migration script which assigns numbers in
 * start_time order within each (convention_date, department) group.
 *
 ...
 * @returns {string} Uppercase code, e.g. "FRLG1"
 */
export function generateShiftCode(conventionDate, department, sequenceNumber) {
