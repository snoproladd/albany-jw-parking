...
export async function getSigns({ includeArchived = false } = {}) {
    ...
    const result = await exec(`
        SELECT
            ...
            s.updated_at,
            s.is_archived,
            (SELECT COUNT(*) FROM dbo.sign_attachments a WHERE a.sign_id = s.sign_id) AS attachment_count
        FROM dbo.signs s
        ${where}
 ...
 * @returns {Promise<void>}
 */
export async function setSignLocationPhoto(locationId, blobName, actorName) {
    await exec(`
        UPDATE dbo.sign_locations
...
/**
 * Update a traffic arrow's position, bearing, label, and colour.
 *
