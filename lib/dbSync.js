 ...
 * Optionally filtered by sign or by status.
 *
 * @param {{
 *   signId?: number|null,
 *   status?: string|null,
 ...
 * @returns {Promise<Array<object>>}
 */
export async function getSignPlacements({ signId = null, status = null } = {}) {
    const where = [];
...
export async function createSignPlacement({
    ...
    const result = await exec(`
        INSERT INTO dbo.sign_placements
            (sign_id, latitude, longitude, heading, location_notes, status, created_by)
        OUTPUT INSERTED.placement_id
        VALUES
            (@signId, @latitude, @longitude, @heading, @locationNotes, @status, @createdBy);
    `, (req) => {
        req.input('signId',        sql.Int,            signId);
