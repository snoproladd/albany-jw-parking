 ...
 * Performs normalized update and accepts SMS capable flag (stored later if added).
 */
export async function insertNameAndPhone(id, firstName, lastName, phone, suffix, smsCapableRaw) {
  ...
  `;
  const result = await exec(sqlText, (req) => {
    // Note: SMSCapable is accepted to keep API surface stable, but not stored in the current schema.
