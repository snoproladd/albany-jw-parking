...
// Load/Reload volunteer cache (dbo.volunteer_in)
...
export async function insertCongregationInfo(
  ...
  // Normalize congAssigned ("yes"/"no")
  const congAssigned =
    String(congAssignedRaw).toLowerCase() === 'yes';

  // Normalize extraAttend checkbox
  const extraAttend =
    extraAttendRaw === true ||
    extraAttendRaw === 1 ||
    extraAttendRaw === '1' ||
  ...
  if (congAssigned) {
    // assigned → use dropdown
    congregationValue = congregation ?? null;
  } else {
    // visiting → build string
...
export async function insertNameAndPhone(id, firstName, lastName, phone, suffix, SMSCapable) {
  console.log('SMSCapable: ',SMSCapable)
  const sqlText = `
    UPDATE dbo.volunteer_in 
...
/**
 * Check if a volunteer already exists by name OR phone.
 * Prevents duplicates by name (regardless of phone) and enforces unique phone.
 */
export async function namePhoneExists(firstName, lastName, phone, suffix) {
  ...
  return res.recordset?.[0] ?? null; // Return the existing record if found
};


export async function getCongregations() {
  const tsql = `SELECT * FROM dbo.congregations`;
  try {
    const res = await exec(tsql);
    const congregations = res.recordset?.map(item => `${item.Congregation}, ${item.State}`) ?? [];
