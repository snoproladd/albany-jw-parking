...
export async function getActiveVolunteers() {
  const result = await exec(
    `
    SELECT id, firstName, lastName, suffix, registration_status, active_current_year
    FROM dbo.volunteer_in
    ...
    const result = await exec(`
        INSERT INTO dbo.invitation_batches
            (name, convention_day_id, shift_id, message_subject,
             message_body, year, created_by)
        OUTPUT INSERTED.id
        VALUES
            (@name, @conventionDayId, @shiftId, @messageSubject,
             @messageBody, @year, @createdBy);
    `, (req) => {
