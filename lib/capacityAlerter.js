 ...
 * again. The disarm claim (see lib/dbSync.js's disarmCapacityAlertRule)
 * is also gated by a minimum cooldown independent of is_armed, as a
 * second line of defense against repeat-firing if is_armed is ever
 * wrong -- see the 2026-07-03 incident in CHANGELOG.md.
 *
 * Threshold types:
...
export async function evaluateCapacityAlerts({
  ...
  // Global kill switch, checked before anything else. Fails CLOSED: if
  // this read itself errors, skip evaluation entirely rather than risk
  // sending -- silence is the safe default for an alert system.
  try {
    const killSwitch = await getCapacityAlertKillSwitch();
    if (killSwitch.enabled) return;
  } catch (e) {
    err("[capacityAlerter] failed to read kill switch, skipping evaluation:", e);
    return;
  }
