...
export async function getActiveCapacityAlertRulesForLocation(locationTaskId, subLocationId) {
    const result = await exec(
        `SELECT   r.id, r.location_task_id, r.sub_location_id, r.threshold_type,
                  ...
                  r.message_override, r.is_armed, lt.capacity
 ...
 * armed to disarmed. Uses a conditional UPDATE (WHERE is_armed = 1) so
 * concurrent evaluations of the same rule -- e.g. overlapping heartbeats
 * while a prior evaluation is still mid-send -- can only have one caller
 * win the race. The loser sees rowsAffected === 0 and must skip sending.
 *
 * @param {number} ruleId
