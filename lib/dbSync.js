...
/**
 * Fetch all hierarchy nodes as a flat list, sorted for tree rendering.
 *
 * @returns {Promise<Array<{id, parent_id, role_title, sort_order, volunteer_id, firstName, lastName, phone}>>}
 */
export async function getCommandHierarchy() {
    const result = await exec(
        `SELECT
             ch.id,
             ch.parent_id,
 ...
 */
export async function addHierarchyNode({ volunteer_id, parent_id, role_title, sort_order }) {
    const result = await exec(
        `INSERT INTO dbo.command_hierarchy (volunteer_id, parent_id, role_title, sort_order)
         OUTPUT INSERTED.id
...
/**
 * Bulk-save the full hierarchy (sort_order + parent_id for every node).
 ...
 * @returns {Promise<void>}
 */
export async function saveHierarchyOrder(nodes) {
    for (const n of nodes) {
        await exec(
            `UPDATE dbo.command_hierarchy
...
export async function deleteHierarchyNode(id) {
    const nodeRes = await exec(
        `SELECT parent_id FROM dbo.command_hierarchy WHERE id = @id;`,
        (req) => req.input('id', sql.Int, id),
    );
    const parentId = nodeRes.recordset?.[0]?.parent_id ?? null;

    await exec(
        `UPDATE dbo.command_hierarchy SET parent_id = @parentId WHERE parent_id = @id;`,
        (req) => { req.input('parentId', sql.Int, parentId); req.input('id', sql.Int, id); },
    );
    await exec(
        `DELETE FROM dbo.command_hierarchy WHERE id = @id;`,
        (req) => req.input('id', sql.Int, id),
    );
}
