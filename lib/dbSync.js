...
/**
 * Insert a record into `published_files`.
 *
 * @param {{
 *   fileType:       string,
 *   filename:       string,
...
export async function insertPublishedFile(data) {
    ...
    `, (req) => {
        ...
        req.input('sharePointUrl', sql.NVarChar(1000), data.sharePointUrl || null);
        req.input('publishedBy',   sql.NVarChar(255),  data.publishedBy);
    });
    return result.recordset?.[0]?.id;
}

/**
 * Get all published files, optionally filtered by type.
 *
 * @param {string} [fileType] - e.g. 'sign-map'. Omit for all types.
 * @returns {Promise<Array>}
 */
export async function getPublishedFiles(fileType) {
    const result = await exec(`
        SELECT id, file_type, filename, blob_name, sharepoint_url,
               published_by, created_at
        FROM dbo.published_files
        ${fileType ? 'WHERE file_type = @fileType' : ''}
        ORDER BY created_at DESC;
    `, (req) => {
        if (fileType) req.input('fileType', sql.NVarChar(50), fileType);
    });
    return result.recordset || [];
}