 ...
 */
function _buildBatchNotification(vol, dayInfos) {
    ...
    // ── SMS ──────────────────────────────────────────────────
    const smsParts = ['Albany JW Parking: Schedule update.'];

    for (const di of dayInfos) {
        if (di.isRemoved) {
            smsParts.push(`${di.dayLabel}: no longer scheduled.`);
        } else if (di.assignments.length) {
            const shiftStr = di.assignments
                .map((a) => `${a.shift_label} at ${a.location_name} (${a.start_time})`)
                .join(', ');
            smsParts.push(`${di.dayLabel}: ${shiftStr}.`);
        }
    }

    if (dayInfos.length === 1) {
        smsParts.push(`View: ${dayInfos[0].downloadUrl}`);
    } else {
        const links = dayInfos
            .map((di) => `${di.dayLabel}: ${di.downloadUrl}`)
            .join(' ');
        smsParts.push(`Schedules: ${links}`);
    }
