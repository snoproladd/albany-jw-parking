...
export async function deleteMapFilesNotInSourceIds(keepSourceItemIds) {
    ...
    table.columns.add('source_item_id', sql.NVarChar(255));
    keepSourceItemIds.forEach((id) => table.rows.add(id));

    const toDelete = await exec(`
        SELECT source_item_id, blob_name
        FROM dbo.map_files
        WHERE source_item_id NOT IN (SELECT source_item_id FROM @keepIds);
    `, (req) => {
        req.input('keepIds', table);
    });
