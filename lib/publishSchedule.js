...
export async function publishDaySchedule(opts) {
    const {
        ...
        serverPort, appBaseUrl, smtpConfig, twilioAccountSid, twilioAuthToken, twilioMsgSid,
        graphConfig,
        dryRun = false,
    } = opts;
    ...
    // Also upload to Blob Storage for authentication-free delivery to volunteers
    const blobName = await uploadPublishedFile(pdfBuffer, filename);
    const downloadUrl = `${appBaseUrl}/schedule/pdf/${encodeURIComponent(blobName)}`;
    log(`${dryRun ? '[DRY RUN] ' : ''}Blob upload complete, download URL: ${downloadUrl}`);
    ...
    for (const r of recipients) {
        const { subject, emailBody, smsBody } = buildNotificationContent(
            r.firstName, dayLabel, conventionDate, r.assignments, downloadUrl,
        );
