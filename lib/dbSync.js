 ...
 */
export async function createSign({ signText, arrowDirection = null, description = null }, createdBy) {
    const result = await exec(`
        INSERT INTO dbo.signs (sign_text, arrow_direction, description, created_by)
        OUTPUT INSERTED.sign_id
        VALUES (@signText, @arrowDirection, @description, @createdBy);
    `, (req) => {
 ...
 * @returns {Promise<boolean>} True if a row was updated.
 */
export async function updateSign(signId, { signText, arrowDirection = null, description = null }) {
    const result = await exec(`
        UPDATE dbo.signs
