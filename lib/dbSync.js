 ...
 *   volunteer_need?:number|null, notes?:string|null}} data
 * @returns {Promise<number>}
 */
  ...
  const result = await exec(
    `
        INSERT INTO dbo.shifts
            (session_id, event_type_id, label, start_time, end_time, volunteer_need, notes)
        OUTPUT INSERTED.id
        VALUES (@sessionId, @eventTypeId, @label, @start_time, @end_time, @volunteer_need, @notes);
    `,
    (req) => {
 ...
 *   end_time:string, volunteer_need?:number|null, notes?:string|null}} data
 * @returns {Promise<boolean>}
 */
...
export async function createScheduleAssignment(data) {
  const result = await exec(
    `
        INSERT INTO dbo.schedule_assignments (shift_id, location_task_id, volunteer_need, notes)
        OUTPUT INSERTED.id
        VALUES (@shiftId, @locationTaskId, @volunteerNeed, @notes);
    `,
    (req) => {
...
export async function getShiftAttendanceData(shiftId) {
    const result = await exec(`
        -- Invited volunteers for this shift
        SELECT
            v.id            AS volunteer_id,
            v.firstName,
            v.lastName,
            i.response      AS rsvp_response,
        ...
        LEFT JOIN dbo.attendance a
            ON a.volunteer_id = i.volunteer_id
...
export async function getAttendanceDayData(dayId) {
    const result = await exec(`
        -- Volunteers invited to this day (no specific shift)
        SELECT
            v.id            AS volunteer_id,
            v.firstName,
            v.lastName,
        ...
        WHERE i.convention_day_id = @dayId
          AND i.shift_id          IS NULL
          AND i.revoked            = 0
