...
export async function getActiveVolunteers() {
  const result = await exec(
    `
    SELECT id, firstName, lastName, suffix, active_current_year
    FROM dbo.volunteer_in
  ...
  for (let v = 0; v < result.recordset.length; v++) {
    const { id, firstName, lastName, suffix, active_current_year } = result.recordset[v];
    volunteers.push({ id, lastName, firstName, suffix, active: !!active_current_year });
  }
  return volunteers;
}
 ...
 * @returns {Promise<boolean>} true if a row was updated
 */
export async function updateVolunteerAssignment(targetId, newRole, crews, editedBy) {
    const result = await exec(`
        UPDATE dbo.volunteer_in
        SET role                = @role,
