  ...
  const result = await exec(
    `
        INSERT INTO dbo.shifts
            (session_id, event_type_id, label, start_time, end_time, department, volunteer_need, notes, sms_code)
        OUTPUT INSERTED.id
        VALUES (@sessionId, @eventTypeId, @label, @start_time, @end_time, @department, @volunteer_need, @notes, @smsCode);;
    `,
    (req) => {
 ...
 *   end_time:string, department?:string|null, volunteer_need?:number|null,
 *   notes?:string|null, sms_code?:string|null}} data
  * @returns {Promise<boolean>}
  */
...
export async function getVolunteerShiftsForDay(volunteerId, dayId) {
    ...
    return shiftRes.recordset.map((r) => ({
        ...
        shift_label:   r.shift_label,
        dept_key:      r.dept_key,
        dept_name:     DEPT_NAMES[r.dept_key] || r.dept_key,
        start_time:    fmtT(r.shift_start),
        end_time:      fmtT(r.shift_end),
        location_name: r.location_name,
...
export async function getSchedulePreview(params) {
    ...
    const result = await exec(`
        ...
        GROUP BY
            sh.id, sh.label, sh.start_time, sh.department,
            cd.convention_date, cd.label, et.name
        ORDER BY cd.convention_date, sh.start_time;
    `, (req) => {
        req.input('year',            sql.Int,           year);
        req.input('scheduleId',      sql.Int,           scheduleId);
...
export async function getSigns({ includeArchived = false } = {}) {
    ...
    const result = await exec(`
        SELECT
            ...
            s.updated_at,
            s.is_archived,
            (SELECT COUNT(*) FROM dbo.sign_placements p WHERE p.sign_id = s.sign_id) AS placement_count
        FROM dbo.signs s
        ${where}
...
export async function getSignById(signId) {
    ...
    const placeRes = await exec(
        `SELECT * FROM dbo.sign_placements WHERE sign_id = @signId ORDER BY created_at;`,
        (req) => req.input('signId', sql.Int, signId),
    );
    sign.placements = placeRes.recordset || [];
    return sign;
}
...
export async function createSign(
    { signText, arrowDirection = null, abbreviation = null, description = null },
    createdBy,
) {
    const result = await exec(`
        INSERT INTO dbo.signs (sign_text, arrow_direction, abbreviation, description, created_by)
        OUTPUT INSERTED.sign_id
        VALUES (@signText, @arrowDirection, @abbreviation, @description, @createdBy);
    `, (req) => {
 ...
 * Existing placements pointing at this sign remain intact; the FK keeps
 * historical placements valid even after the template is hidden from the picker.
 *
 * @param {number} signId
 * @returns {Promise<boolean>}
 */
...
/**
 * Fetch all sign placements, joined to their template for display.
 * Optionally filtered by sign or by status. By default excludes
 * placements of archived sign templates; pass `includeArchived: true`
 * to include them.
 *
 * @param {{
 *   signId?:          number|null,
 ...
 * @returns {Promise<Array<object>>}
 */
export async function getSignPlacements({
    signId = null,
    status = null,
    includeArchived = false,
} = {}) {
    const where = [];
    if (signId)          where.push('p.sign_id = @signId');
    if (status)          where.push('p.status = @status');
    ...
    const result = await exec(`
        SELECT
            ...
            p.created_at,
            p.updated_at
        FROM dbo.sign_placements p
        INNER JOIN dbo.signs s ON s.sign_id = p.sign_id
        ${whereSql}
        ORDER BY p.created_at DESC;
    `, (req) => {
        if (signId) req.input('signId', sql.Int,          signId);
        if (status) req.input('status', sql.NVarChar(20), status);
    });
    const rows = result.recordset || [];
    // Compact map markers display this abbreviation when the disc is too
    // small for the full sign text. NULL override falls back to the
    // heuristic so existing rows work without a backfill migration.
    return rows.map((r) => ({
        ...r,
        abbreviation: r.abbreviation || computeSignAbbreviation(r.sign_text),
    }));
...
/**
 * Fetch one placement with its sign template fields joined.
 *
 * @param {number} placementId
 * @returns {Promise<object|null>}
 */
export async function getSignPlacementById(placementId) {
    const result = await exec(`
        SELECT
            p.placement_id,
            p.sign_id,
            ...
            s.description AS sign_description,
            p.latitude,
            p.longitude,
            p.heading,
            p.location_notes,
        ...
        FROM dbo.sign_placements p
        INNER JOIN dbo.signs s ON s.sign_id = p.sign_id
        WHERE p.placement_id = @placementId;
    `, (req) => req.input('placementId', sql.Int, placementId));
    return result.recordset?.[0] || null;
}

/**
 * Insert a new placement for an existing sign template.
 *
 * @param {{
 *   signId:              number,
 *   latitude:            number,
 *   longitude:           number,
 *   heading?:            number|null,  Direction of travel (0–360°, clockwise from north). The
 *                                      bearing drivers travel *toward* the sign. Drives the travel-
 *                                      direction arrow on the map marker and Street View positioning.
 *                                      Null = not set.
 ...
 * }} data
 * @param {string} createdBy
 * @returns {Promise<number>} The new placement_id.
 */
export async function createSignPlacement({
    signId,
    latitude,
    longitude,
    heading = null,
    ...
    const result = await exec(`
        INSERT INTO dbo.sign_placements
            (sign_id, latitude, longitude, heading, location_notes, status, mount_type, marker_color, arrow_direction, created_by)
        OUTPUT INSERTED.placement_id
        VALUES
            (@signId, @latitude, @longitude, @heading, @locationNotes, @status, @mountType, @markerColor, @arrowDirection, @createdBy);
    `, (req) => {
        req.input('signId',          sql.Int,            signId);
        req.input('latitude',        sql.Decimal(10, 7), latitude);
        req.input('longitude',       sql.Decimal(10, 7), longitude);
        req.input('heading',         sql.Decimal(5, 2),  heading);
        req.input('locationNotes',   sql.NVarChar(500),  locationNotes || null);
        req.input('status',          sql.NVarChar(20),   status || 'planned');
        req.input('mountType',       sql.NVarChar(30),   mountType || null);
        req.input('markerColor',     sql.NVarChar(20),   markerColor || null);
        req.input('arrowDirection',  sql.NVarChar(20),   arrowDirection || null);
        req.input('createdBy',       sql.NVarChar(100),  createdBy);
    });

    const id = result.recordset?.[0]?.placement_id;
    if (!id) throw new Error('INSERT sign_placements did not return placement_id.');
    return id;
}

/**
 * Update editable fields on a placement.
 * Does NOT change status — use updateSignPlacementStatus for that
 * so the installed/removed audit columns stay in sync.
 *
 * @param {number} placementId
 * @param {{
 *   latitude:            number,
 *   longitude:           number,
 *   heading?:            number|null,  Direction of travel (0–360°). See createSignPlacement.
 *   locationNotes?:      string|null,
 *   mountType?:          string|null,
 * }} data
 * @returns {Promise<boolean>}
 */
export async function updateSignPlacement(
  placementId,
  {
    latitude,
    longitude,
  ...
  const result = await exec(
    ...
    `,
    (req) => {
      req.input("placementId", sql.Int, placementId);
      req.input("latitude", sql.Decimal(10, 7), latitude);
      req.input("longitude", sql.Decimal(10, 7), longitude);
      req.input("heading", sql.Decimal(5, 1), heading);
      req.input("locationNotes", sql.NVarChar(500), locationNotes || null);
      req.input("mountType", sql.NVarChar(30), mountType || null);
      req.input("markerColor", sql.NVarChar(20), markerColor || null);
      req.input("arrowDirection", sql.NVarChar(20), arrowDirection || null);
...
/**
 * Update only the status field of a placement, automatically setting
 * installed_by / installed_at when transitioning to 'installed' and
 * removed_at when transitioning to 'removed'. Resetting to 'planned'
 * clears the installation/removal audit columns.
 *
 * @param {number} placementId
 * @param {string} status - One of 'planned', 'installed', 'removed'.
 * @param {string} editedBy
 * @returns {Promise<boolean>}
 */
export async function updateSignPlacementStatus(placementId, status, editedBy) {
    const validStatuses = ['planned', 'installed', 'removed'];
    if (!validStatuses.includes(status)) {
        throw new Error(`Invalid placement status: ${status}`);
    }

    // Build the SET clause based on the destination status so the audit
    // columns (installed_by / installed_at / removed_at) stay coherent.
    let sets = `status = @status, updated_at = SYSUTCDATETIME()`;
    if (status === 'installed') {
        sets += `, installed_by = @editedBy, installed_at = SYSUTCDATETIME(), removed_at = NULL`;
    } else if (status === 'removed') {
        sets += `, removed_at = SYSUTCDATETIME()`;
    } else {
        // Back to 'planned' — clear installation/removal audit columns
        sets += `, installed_by = NULL, installed_at = NULL, removed_at = NULL`;
    }
...
/**
 * Hard-delete a placement. The template remains intact.
 *
 * @param {number} placementId
 * @returns {Promise<boolean>}
 */
export async function deleteSignPlacement(placementId) {
    const result = await exec(
        `DELETE FROM dbo.sign_placements WHERE placement_id = @placementId;`,
        (req) => req.input('placementId', sql.Int, placementId),
    );
    return (result.rowsAffected?.[0] ?? 0) > 0;
}
/**
 * Set the photo_url (blob name) for a placement and record who
 * took / uploaded it.
 *
 * @param {number}      placementId
 * @param {string}      blobName   The blob name returned by uploadSignPhoto.
 * @param {string|null} actorName  Display name of the person (e.g. "Jake Ladd").
 * @returns {Promise<boolean>}
 */
export async function setSignPlacementPhoto(placementId, blobName, actorName, svState = null) {
    const result = await exec(`
        UPDATE dbo.sign_placements
    ...
    `, (req) => {
        ...
        req.input('actorName',   sql.NVarChar(200), actorName || null);
        req.input('hasSvState',  sql.Bit,           svState != null ? 1 : 0);
        req.input('svPanoId',    sql.NVarChar(100), svState?.panoId  ?? null);
        req.input('svHeading',   sql.Decimal(6, 2), svState?.heading != null ? Number(svState.heading) : null);
        req.input('svPitch',     sql.Decimal(6, 2), svState?.pitch   != null ? Number(svState.pitch)   : null);
        req.input('svFov',       sql.Decimal(6, 2), svState?.fov     != null ? Number(svState.fov)     : null);
    });
    return (result.rowsAffected?.[0] ?? 0) > 0;
}

/**
 * Clear the photo_url and credit fields for a placement
 * (used after blob deletion).
 *
 * @param {number} placementId
 * @returns {Promise<boolean>}
 */
export async function clearSignPlacementPhoto(placementId) {
    const result = await exec(`
        UPDATE dbo.sign_placements
        SET photo_url      = NULL,
            photo_taken_by = NULL,
...
/**
 * Set marker_color on every placement belonging to a sign template.
 * Pass null to clear all custom colours and revert to status-based.
 *
 * @param {number} signId
 * @param {string|null} markerColor
 * @returns {Promise<number>} Number of rows updated.
 */
export async function bulkSetPlacementColor(signId, markerColor) {
    const result = await exec(`
        UPDATE dbo.sign_placements
        SET marker_color = @markerColor,
            updated_at   = SYSUTCDATETIME()
        WHERE sign_id = @signId;
    `, (req) => {
        req.input('signId',      sql.Int,          signId);
