...
export async function createSignPlacement({
    ...
    const result = await exec(`
        INSERT INTO dbo.sign_placements
            (sign_id, latitude, longitude, heading, location_notes, status, mount_type, created_by)
        OUTPUT INSERTED.placement_id
        VALUES
            (@signId, @latitude, @longitude, @heading, @locationNotes, @status, @mountType, @createdBy);
    `, (req) => {
        req.input('signId',        sql.Int,            signId);
 ...
 * @returns {Promise<boolean>}
 */
export async function updateSignPlacement(placementId, {
    latitude,
    longitude,
    heading = null,
    locationNotes = null,
