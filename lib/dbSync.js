...
/**
 * Set the photo_url (blob name) for a placement.
 *
 * @param {number} placementId
 * @param {string} blobName  The blob name returned by uploadSignPhoto.
 * @returns {Promise<boolean>}
 */
export async function setSignPlacementPhoto(placementId, blobName) {
    const result = await exec(`
        UPDATE dbo.sign_placements
...
/**
 * Clear the photo_url for a placement (used after blob deletion).
 *
 * @param {number} placementId
 * @returns {Promise<boolean>}
 */
